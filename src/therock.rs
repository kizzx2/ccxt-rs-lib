...
pub trait Therock : Exchange {
    ...
    async fn fetch_markets(&mut self, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "publicGetFunds".into(), params.clone(), Value::Undefined).await;
        //
        //    {
        //        funds: [
        //            {
        ...
        //    }
        //
        let mut markets: Value = self.safe_value(response.clone(), Value::from("funds"), Value::Undefined);
        let mut result: Value = Value::new_array();
        if markets.clone().is_nullish() {
            panic!(r###"ExchangeError::new(self.get("id".into()) + Value::from(" fetchMarkets() got an unexpected response"))"###);
        } else {
            let mut i: usize = 0;
            while i < markets.len() {
                let mut market: Value = markets.get(i.into());
                let mut id: Value = self.safe_string(market.clone(), Value::from("id"), Value::Undefined);
                let mut base_id: Value = self.safe_string(market.clone(), Value::from("trade_currency"), Value::Undefined);
                let mut quote_id: Value = self.safe_string(market.clone(), Value::from("base_currency"), Value::Undefined);
                let mut base: Value = Therock::safe_currency_code(self, base_id.clone(), Value::Undefined);
                let mut quote: Value = Therock::safe_currency_code(self, quote_id.clone(), Value::Undefined);
                let mut buy_fee: Value = self.safe_string(market.clone(), Value::from("buy_fee"), Value::Undefined);
                let mut sell_fee: Value = self.safe_string(market.clone(), Value::from("sell_fee"), Value::Undefined);
                let mut taker: Value = Precise::string_max(buy_fee.clone(), sell_fee.clone());
                taker = self.parse_number(Precise::string_div(taker.clone(), Value::from("100"), Value::Undefined), Value::Undefined);
                let mut leverages: Value = self.safe_value(market.clone(), Value::from("leverages"), Value::Undefined);
                let mut leverages_length: Value = leverages.len().into();
                result.push(Value::Json(normalize(&Value::Json(json!({
                    ...
                    "type": "spot",
                    "spot": true,
                    "margin": leverages_length.clone() > Value::from(0),
                    "swap": false,
                    "future": false,
                    "option": false,
                    "contract": false,
                    ...
                    "precision": Value::Json(normalize(&Value::Json(json!({
                        "amount": self.parse_number(Therock::parse_precision(self, self.safe_string(market.clone(), Value::from("trade_currency_decimals"), Value::Undefined)), Value::Undefined),
                        "price": self.parse_number(Therock::parse_precision(self, self.safe_string(market.clone(), Value::from("base_currency_decimals"), Value::Undefined)), Value::Undefined)
                    }))).unwrap()),
                    "limits": Value::Json(normalize(&Value::Json(json!({
                        "leverage": Value::Json(normalize(&Value::Json(json!({
                            "min": 1,
                            "max": self.safe_value(leverages.clone(), leverages_length.clone() - Value::from(1), Value::from(1))
                        }))).unwrap()),
                        "amount": Value::Json(normalize(&Value::Json(json!({
                            "min": Therock::safe_number(self, market.clone(), Value::from("minimum_quantity_offer"), Value::Undefined),
                            "max": Value::Undefined
                        }))).unwrap()),
                        "price": Value::Json(normalize(&Value::Json(json!({
                            "min": Therock::safe_number(self, market.clone(), Value::from("minimum_price_offer"), Value::Undefined),
                            "max": Value::Undefined
                        }))).unwrap()),
    ...
    fn parse_balance(&self, mut response: Value) -> Value {
        ...
        while i < balances.len() {
            ...
            let mut code: Value = Therock::safe_currency_code(self, currency_id.clone(), Value::Undefined);
            let mut account: Value = Therock::account(self);
            account.set("free".into(), self.safe_string(balance.clone(), Value::from("trading_balance"), Value::Undefined));
            account.set("total".into(), self.safe_string(balance.clone(), Value::from("balance"), Value::Undefined));
            result.set(code.clone(), account.clone());
            i += 1;
        };
        return Therock::safe_balance(self, result.clone());
    }
    ...
    /// # Arguments
    ///
    /// * `params` {object} - extra parameters specific to the therock api endpoint
    async fn fetch_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut response: Value = Therock::dispatch(self, "privateGetBalances".into(), params.clone(), Value::Undefined).await;
        return Therock::parse_balance(self, response.clone());
    }
    ...
    async fn fetch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut market: Value = Therock::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "id": market.get(Value::from("id"))
        }))).unwrap());
        let mut orderbook: Value = Therock::dispatch(self, "publicGetFundsIdOrderbook".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        let mut timestamp: Value = self.parse8601(self.safe_string(orderbook.clone(), Value::from("date"), Value::Undefined));
        return Therock::parse_order_book(self, orderbook.clone(), market.get(Value::from("symbol")), timestamp.clone(), Value::from("bids"), Value::from("asks"), Value::from("price"), Value::from("amount"));
    }
    ...
    fn parse_ticker(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        //     }
        //
        let mut timestamp: Value = self.parse8601(self.safe_string(ticker.clone(), Value::from("date"), Value::Undefined));
        market = Therock::safe_market(self, Value::Undefined, market.clone(), Value::Undefined);
        let mut last: Value = self.safe_string(ticker.clone(), Value::from("last"), Value::Undefined);
        return Therock::safe_ticker(self, Value::Json(normalize(&Value::Json(json!({
            ...
            "close": last,
            "last": last,
            "previousClose": self.safe_string(ticker.clone(), Value::from("close"), Value::Undefined),
            "change": Value::Undefined,
            "percentage": Value::Undefined,
            "average": Value::Undefined,
            "baseVolume": self.safe_string(ticker.clone(), Value::from("volume_traded"), Value::Undefined),
    ...
    async fn fetch_tickers(&mut self, mut symbols: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        symbols = Therock::market_symbols(self, symbols.clone());
        let mut response: Value = Therock::dispatch(self, "publicGetFundsTickers".into(), params.clone(), Value::Undefined).await;
        let mut tickers: Value = self.index_by(response.get(Value::from("tickers")), Value::from("fund_id"), Value::Undefined);
        let mut ids: Value = Object::keys(tickers.clone());
        let mut result: Value = Value::new_object();
        let mut i: usize = 0;
        while i < ids.len() {
            let mut id: Value = ids.get(i.into());
            let mut market: Value = Therock::safe_market(self, id.clone(), Value::Undefined, Value::Undefined);
            let mut symbol: Value = market.get(Value::from("symbol"));
            let mut ticker: Value = tickers.get(id.clone());
            result.set(symbol.clone(), Therock::parse_ticker(self, ticker.clone(), market.clone()));
            i += 1;
        };
        return Therock::filter_by_array(self, result.clone(), Value::from("symbol"), symbols.clone(), Value::Undefined);
    }
    ...
    async fn fetch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "publicGetFundsIdTicker".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "date":"2022-01-16T00:05:08.192Z",
        //         "fund_id":"ETHBTC",
        ...
        //     }
        //
        return Therock::parse_ticker(self, response.clone(), market.clone());
    }
    ...
    fn parse_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        //                         trade_id:  440492                     }   ] }
        //
        let mut market_id: Value = self.safe_string(trade.clone(), Value::from("fund_id"), Value::Undefined);
        let mut symbol: Value = Therock::safe_symbol(self, market_id.clone(), market.clone(), Value::Undefined);
        ...
        let mut transactions: Value = self.safe_value(trade.clone(), Value::from("transactions"), Value::new_array());
        let mut transactions_by_type: Value = self.group_by(transactions.clone(), Value::from("type"), Value::Undefined);
        let mut fee_transactions: Value = self.safe_value(transactions_by_type.clone(), Value::from("paid_commission"), Value::new_array());
        let mut i: usize = 0;
        while i < fee_transactions.len() {
            if fee_cost_string.clone().is_nullish() {
                fee_cost_string = Value::from("0.0");
            };
            fee_cost_string = Precise::string_add(fee_cost_string.clone(), self.safe_string(fee_transactions.get(i.into()), Value::from("price"), Value::Undefined));
            i += 1;
        };
        ...
        return Therock::safe_trade(self, Value::Json(normalize(&Value::Json(json!({
            "info": trade,
            "id": id,
            "order": order_id,
            "timestamp": timestamp,
    ...
    fn parse_ledger_entry_direction(&self, mut direction: Value) -> Value {
    ...
    fn parse_ledger_entry(&self, mut item: Value, mut currency: Value) -> Value {
        ...
        let mut direction: Value = Therock::parse_ledger_entry_direction(self, r#type.clone());
        r#type = Therock::parse_ledger_entry_type(self, r#type.clone());
        if r#type.clone() == Value::from("trade") || r#type.clone() == Value::from("fee") {
            reference_id = self.safe_string(item.clone(), Value::from("trade_id"), Value::Undefined);
        };
        let mut currency_id: Value = self.safe_string(item.clone(), Value::from("currency"), Value::Undefined);
        let mut code: Value = Therock::safe_currency_code(self, currency_id.clone(), Value::Undefined);
        let mut amount: Value = Therock::safe_number(self, item.clone(), Value::from("price"), Value::Undefined);
        let mut timestamp: Value = self.parse8601(self.safe_string(item.clone(), Value::from("date"), Value::Undefined));
    ...
    async fn fetch_ledger(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if code.clone().is_nonnullish() {
            currency = Therock::currency(self, code.clone());
            request.set("currency".into(), currency.get(Value::from("id")));
        };
        if since.clone().is_nonnullish() {
            request.set("after".into(), self.iso8601(since.clone()));
        };
        let mut response: Value = Therock::dispatch(self, "privateGetTransactions".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "transactions": [
        //             {
        ...
        //     }
        //
        let mut transactions: Value = self.safe_value(response.clone(), Value::from("transactions"), Value::new_array());
        return Therock::parse_ledger(self, transactions.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    fn parse_transaction(&self, mut transaction: Value, mut currency: Value) -> Value {
        ...
        //
        let mut id: Value = self.safe_string_2(transaction.clone(), Value::from("id"), Value::from("transaction_id"), Value::Undefined);
        let mut r#type: Value = Therock::parse_transaction_type(self, self.safe_string(transaction.clone(), Value::from("type"), Value::Undefined));
        let mut detail: Value = self.safe_value(transaction.clone(), Value::from("transfer_detail"), Value::new_object());
        let mut method: Value = self.safe_string(detail.clone(), Value::from("method"), Value::Undefined);
        let mut txid: Value = Value::Undefined;
        let mut address: Value = Value::Undefined;
        if method.clone().is_nonnullish() {
            if method.clone() != Value::from("wire_transfer") {
                txid = self.safe_string(detail.clone(), Value::from("id"), Value::Undefined);
                address = self.safe_string(detail.clone(), Value::from("recipient"), Value::Undefined);
            };
        };
        let mut currency_id: Value = self.safe_string(transaction.clone(), Value::from("currency"), Value::Undefined);
        let mut code: Value = Therock::safe_currency_code(self, currency_id.clone(), Value::Undefined);
        let mut amount: Value = Therock::safe_number(self, transaction.clone(), Value::from("price"), Value::Undefined);
        let mut timestamp: Value = self.parse8601(self.safe_string(transaction.clone(), Value::from("date"), Value::Undefined));
        let mut status: Value = Value::from("ok");
        let mut network: Value = self.safe_string(detail.clone(), Value::from("method"), Value::Undefined);
        // todo parse tags
        return Value::Json(normalize(&Value::Json(json!({
            "info": transaction,
            "id": id,
    ...
    async fn fetch_withdrawals(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return Therock::fetch_transactions(self, code.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
    }
    ...
    async fn fetch_transactions(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "privateGetTransactions".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "transactions": [
        //             {
        ...
        //
        let mut transactions: Value = self.safe_value(response.clone(), Value::from("transactions"), Value::new_array());
        let mut transaction_types: Value = Value::Json(serde_json::Value::Array(vec![Value::from("withdraw").into(), Value::from("atm_payment").into()]));
        let mut deposits_and_withdrawals: Value = Therock::filter_by_array(self, transactions.clone(), Value::from("type"), transaction_types.clone(), false.into());
        return Therock::parse_transactions(self, deposits_and_withdrawals.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        (tag, params) = shift_2(Therock::handle_withdraw_tag_and_params(self, tag.clone(), params.clone()));
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut currency: Value = Therock::currency(self, code.clone());
        amount = Therock::currency_to_precision(self, code.clone(), amount.clone(), Value::Undefined);
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
        ...
        let mut response: Value = Therock::dispatch(self, "privatePostAtmsWithdraw".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //    { "transaction_id": 65088485 }
        //
        let mut transaction: Value = Therock::parse_transaction(self, response.clone(), currency.clone());
        let mut withdraw_options: Value = self.safe_value(self.get("options".into()), Value::from("withdraw"), Value::new_object());
        let mut fill_response_from_request: Value = self.safe_value(withdraw_options.clone(), Value::from("fillResponseFromRequest"), true.into());
        if fill_response_from_request.is_truthy() {
            if transaction.get(Value::from("addressTo")) == address.clone() {
                transaction.set("addressTo".into(), address.clone());
            };
            if transaction.get(Value::from("address")).is_nullish() {
                transaction.set("address".into(), address.clone());
            };
            if transaction.get(Value::from("tagTo")).is_nullish() {
                transaction.set("tagTo".into(), tag.clone());
            };
            if transaction.get(Value::from("tag")).is_nullish() {
                transaction.set("tag".into(), tag.clone());
            };
            if transaction.get(Value::from("amount")).is_nullish() {
    ...
    fn parse_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        let mut market_id: Value = self.safe_string(order.clone(), Value::from("fund_id"), Value::Undefined);
        let mut symbol: Value = Therock::safe_symbol(self, market_id.clone(), market.clone(), Value::Undefined);
        let mut status: Value = Therock::parse_order_status(self, self.safe_string(order.clone(), Value::from("status"), Value::Undefined));
        let mut timestamp: Value = self.parse8601(self.safe_string(order.clone(), Value::from("date"), Value::Undefined));
        ...
        let mut amount: Value = Therock::safe_number(self, order.clone(), Value::from("amount"), Value::Undefined);
        let mut remaining: Value = Therock::safe_number(self, order.clone(), Value::from("amount_unfilled"), Value::Undefined);
        let mut filled: Value = Value::Undefined;
        if amount.clone().is_nonnullish() {
            if remaining.clone().is_nonnullish() {
                filled = amount.clone() - remaining.clone();
            };
        };
        let mut price: Value = Therock::safe_number(self, order.clone(), Value::from("price"), Value::Undefined);
        ...
        if trades.clone().is_nonnullish() {
            let mut num_trades: Value = trades.len().into();
            if num_trades.clone() > Value::from(0) {
                trades = Therock::parse_trades(self, trades.clone(), market.clone(), Value::Undefined, Value::Undefined, Value::Json(normalize(&Value::Json(json!({
                    "orderId": id
                }))).unwrap()));
                ...
                while i < num_trades.clone().into() {
                    ...
                    cost = self.sum(cost.clone(), trade.get(Value::from("cost")));
        ...
        let mut stop_price: Value = Therock::safe_number(self, order.clone(), Value::from("conditional_price"), Value::Undefined);
        return Value::Json(normalize(&Value::Json(json!({
            "id": id,
    ...
    async fn fetch_open_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return Therock::fetch_orders(self, symbol.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
    }
    ...
    async fn fetch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut market: Value = Therock::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "fund_id": market.get(Value::from("id"))
        }))).unwrap());
        ...
        let mut response: Value = Therock::dispatch(self, "privateGetFundsFundIdOrders".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         orders: [
        //             {
        ...
        return Therock::parse_orders(self, orders.clone(), market.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    /// # Arguments
    ///
    /// * `symbol` {strs} - unified symbol of the market the order was made in
    /// * `params` {object} - extra parameters specific to the therock api endpoint
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "privateGetFundsFundIdOrdersId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "id": 4325578,
        //         "fund_id":"BTCEUR",
        ...
        //     }
        //
        return Therock::parse_order(self, response.clone(), Value::Undefined);
    }
    ...
    async fn create_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "privatePostFundsFundIdOrders".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        return Therock::parse_order(self, response.clone(), Value::Undefined);
    }
    ...
    async fn cancel_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "id": id,
            "fund_id": Therock::market_id(self, symbol.clone())
        }))).unwrap());
        let mut response: Value = Therock::dispatch(self, "privateDeleteFundsFundIdOrdersId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        return Therock::parse_order(self, response.clone(), Value::Undefined);
    }
    ...
    /// * `timeframe` {string} - the length of time each candle represents in minutes
    ...
    async fn fetch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut market: Value = Therock::market(self, symbol.clone());
        let mut period_in_seconds: Value = self.parse_timeframe(timeframe.clone());
        let mut period_in_minutes: Value = parse_int(period_in_seconds.clone() / Value::from(60));
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "id": market.get(Value::from("id")),
        ...
        let mut response: Value = Therock::dispatch(self, "publicGetFundsIdOhlcStatistics".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     [
        //         {
        //             "fund_id": "BTCUSDT",
        ...
        //     ]
        //
        return Therock::parse_ohlcvs(self, response.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    fn parse_ohlcv(&self, mut ohlcv: Value, mut market: Value) -> Value {
        ...
        //     }
        //
        let mut date_time: Value = self.safe_string(ohlcv.clone(), Value::from("interval_starts_at"), Value::Undefined);
        return Value::Json(serde_json::Value::Array(vec![self.parse8601(date_time.clone()).into(), Therock::safe_number(self, ohlcv.clone(), Value::from("open"), Value::Undefined).into(), Therock::safe_number(self, ohlcv.clone(), Value::from("high"), Value::Undefined).into(), Therock::safe_number(self, ohlcv.clone(), Value::from("low"), Value::Undefined).into(), Therock::safe_number(self, ohlcv.clone(), Value::from("close"), Value::Undefined).into(), Therock::safe_number(self, ohlcv.clone(), Value::from("base_volume"), Value::Undefined).into()]));
    }
    ...
    async fn fetch_my_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if limit.clone().is_nonnullish() {
            request.set("per_page".into(), limit.clone());
        };
        // default 25 max 200
        if since.clone().is_nonnullish() {
            request.set("after".into(), self.iso8601(since.clone()));
        };
        let mut response: Value = Therock::dispatch(self, "privateGetFundsIdTrades".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "trades": [
        //             {
        ...
        return Therock::parse_trades(self, trades.clone(), market.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "publicGetFundsIdTrades".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         trades: [
        //             {
        ...
        //     }
        //
        return Therock::parse_trades(self, response.get(Value::from("trades")), market.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_trading_fee(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Therock::dispatch(self, "publicGetFundsId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         id: 'ETHBTC',
        ...
        request = Value::Json(normalize(&Value::Json(json!({
            "id": market.get(Value::from("quoteId"))
        }))).unwrap());
        let mut discount: Value = Therock::dispatch(self, "privateGetDiscountsId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "currency":"BTC",
        //         "discount":50.0,
        ...
        //     }
        //
        return Therock::parse_trading_fee(self, response.clone(), discount.clone(), market.clone());
    }
    ...
    async fn fetch_trading_fees(&mut self, mut params: Value) -> Value {
        ...
        //     }
        //
        let mut discounts_response: Value = Therock::dispatch(self, "privateGetDiscounts".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "discounts": [
        //             {
        ...
        //     }
        //
        let mut funds: Value = self.safe_value(response.clone(), Value::from("funds"), Value::new_array());
        let mut discounts: Value = self.safe_value(discounts_response.clone(), Value::from("discounts"), Value::new_array());
        let mut result: Value = Value::new_object();
        let mut i: usize = 0;
        while i < funds.len() {
            let mut fund: Value = funds.get(i.into());
            let mut market_id: Value = self.safe_string(fund.clone(), Value::from("id"), Value::Undefined);
            let mut market: Value = Therock::safe_market(self, market_id.clone(), Value::Undefined, Value::Undefined);
            let mut quote_id: Value = self.safe_value(market.clone(), Value::from("quoteId"), Value::Undefined);
            let mut discount: Value = self.filter_by(discounts.clone(), Value::from("currency"), quote_id.clone(), Value::Undefined);
            let mut fee: Value = Therock::parse_trading_fee(self, fund.clone(), discount.clone(), market.clone());
    ...
    fn parse_trading_fee(&self, mut fee: Value, mut discount: Value, mut market: Value) -> Value {
        let mut market_id: Value = self.safe_string(fee.clone(), Value::from("id"), Value::Undefined);
        let mut taker_string: Value = self.safe_string(fee.clone(), Value::from("buy_fee"), Value::Undefined);
        let mut maker_string: Value = self.safe_string(fee.clone(), Value::from("sell_fee"), Value::Undefined);
        // TotalFee = (100 - discount) * fee / 10000
        let mut discount_string: Value = self.safe_string(discount.clone(), Value::from("discount"), Value::from("0"));
        let mut fee_percentage: Value = Precise::string_sub(Value::from("100"), discount_string.clone());
        let mut taker: Value = self.parse_number(Precise::string_div(Precise::string_mul(taker_string.clone(), fee_percentage.clone()), Value::from("10000"), Value::Undefined), Value::Undefined);
        let mut maker: Value = self.parse_number(Precise::string_div(Precise::string_mul(maker_string.clone(), fee_percentage.clone()), Value::from("10000"), Value::Undefined), Value::Undefined);
        return Value::Json(normalize(&Value::Json(json!({
            "info": fee,
            "symbol": Therock::safe_symbol(self, market_id.clone(), market.clone(), Value::Undefined),
            "maker": maker,
            "taker": taker,
            "percentage": true,
            "tierBased": true
    ...
    fn sign(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value) -> Value {
        ...
        headers = if headers.clone().is_nullish() { Value::new_object() } else { headers.clone() };
        if api.clone() == Value::from("private") {
            Therock::check_required_credentials(self, Value::Undefined);
            if Object::keys(query.clone()).len() > 0 {
                if method.clone() == Value::from("POST") {
                    ...
                    let mut query_string: Value = self.rawencode(query.clone());
                    if query_string.len() > 0 {
                        url = url +  Value::from("?") + query_string.clone();
                    };
                };
            };
            let mut nonce: Value = Therock::nonce(self).to_string();
            let mut auth: Value = nonce.clone() + url.clone();
            headers.set("X-TRT-KEY".into(), self.get("apiKey".into()));
            headers.set("X-TRT-NONCE".into(), nonce.clone());
            headers.set("X-TRT-SIGN".into(), self.hmac(self.encode(auth.clone()), self.encode(self.get("secret".into())), Value::from("sha512"), Value::Undefined));
        } else if api.clone() == Value::from("public") {
            if Object::keys(query.clone()).len() > 0 {
                url = url +  Value::from("?") + self.rawencode(query.clone());
            };
        };
        return Value::Json(normalize(&Value::Json(json!({
            "url": url,
    ...
    fn handle_errors(&mut self, mut http_code: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut request_headers: Value, mut request_body: Value) -> Value {
        ...
        if num_errors.clone() > Value::from(0) {
            ...
            while i < num_errors.clone().into() {
                ...
                Therock::throw_exactly_matched_exception(self, self.get("exceptions".into()).get(Value::from("exact")), message.clone(), feedback.clone());
                Therock::throw_broadly_matched_exception(self, self.get("exceptions".into()).get(Value::from("broad")), message.clone(), feedback.clone());
                i += 1;
            };
    ...
    fn safe_ledger_entry(&self, mut entry: Value, mut currency: Value) -> Value {
        currency = Therock::safe_currency(self, Value::Undefined, currency.clone());
        ...
        if fee.clone().is_nonnullish() {
            fee.set("cost".into(), Therock::safe_number(self, fee.clone(), Value::from("cost"), Value::Undefined));
        };
    ...
    fn set_markets(&mut self, mut markets: Value, mut currencies: Value) -> Value {
        ...
        while i < market_values.len() {
            let mut market: Value = self.deep_extend_4(Therock::safe_market(self, Value::Undefined, Value::Undefined, Value::Undefined), Value::Json(normalize(&Value::Json(json!({
    ...
    fn safe_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        if parse_filled.is_truthy() || parse_cost.is_truthy() || should_parse_fees.is_truthy() {
            ...
            trades = Therock::parse_trades(self, raw_trades.clone(), market.clone(), Value::Undefined, Value::Undefined, Value::Json(normalize(&Value::Json(json!({
        ...
        if should_parse_fees.is_truthy() {
            let mut reduced_fees: Value = if self.get("reduce_fees".into()).is_truthy() { Therock::reduce_fees_by_currency(self, fees.clone()) } else { fees.clone() };
            let mut reduced_length: Value = reduced_fees.len().into();
            let mut i: usize = 0;
            while i < reduced_length.clone().into() {
                reduced_fees.get(i.into()).set("cost".into(), Therock::safe_number(self, reduced_fees.get(i.into()), Value::from("cost"), Value::Undefined));
                if reduced_fees.get(i.into()).contains_key(Value::from("rate")) {
                    reduced_fees.get(i.into()).set("rate".into(), Therock::safe_number(self, reduced_fees.get(i.into()), Value::from("rate"), Value::Undefined));
                };
                i += 1;
            };
            if !parse_fee.is_truthy() && reduced_length.clone() == Value::from(0) {
                ...
                if fee.contains_key(Value::from("rate")) {
                    fee.set("rate".into(), Therock::safe_number(self, fee.clone(), Value::from("rate"), Value::Undefined));
                };
        ...
        while i < trades.len() {
            ...
            entry.set("amount".into(), Therock::safe_number(self, entry.clone(), Value::from("amount"), Value::Undefined));
            entry.set("price".into(), Therock::safe_number(self, entry.clone(), Value::from("price"), Value::Undefined));
            entry.set("cost".into(), Therock::safe_number(self, entry.clone(), Value::from("cost"), Value::Undefined));
    ...
    fn parse_orders(&mut self, mut orders: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(orders.clone()).is_truthy() {
            let mut i: usize = 0;
            while i < orders.len() {
                let mut order: Value = extend_2(Therock::parse_order(self, orders.get(i.into()), market.clone()), params.clone());
            ...
            while i < ids.len() {
                let mut id: Value = ids.get(i.into());
                let mut order: Value = extend_2(Therock::parse_order(self, extend_2(Value::Json(normalize(&Value::Json(json!({
                    "id": id
        ...
        return Therock::filter_by_symbol_since_limit(self, results.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn safe_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        if should_parse_fees.is_truthy() {
            ...
            let mut trade_fee: Value = self.safe_value(trade.clone(), Value::from("fee"), Value::Undefined);
            if trade_fee.clone().is_nonnullish() {
                trade_fee.set("cost".into(), Therock::safe_number(self, trade_fee.clone(), Value::from("cost"), Value::Undefined));
                if trade_fee.contains_key(Value::from("rate")) {
                    trade_fee.set("rate".into(), Therock::safe_number(self, trade_fee.clone(), Value::from("rate"), Value::Undefined));
                };
    ...
    fn safe_ticker(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        return extend_2(ticker.clone(), Value::Json(normalize(&Value::Json(json!({
            "bid": Therock::safe_number(self, ticker.clone(), Value::from("bid"), Value::Undefined),
            "bidVolume": Therock::safe_number(self, ticker.clone(), Value::from("bidVolume"), Value::Undefined),
            "ask": Therock::safe_number(self, ticker.clone(), Value::from("ask"), Value::Undefined),
            "askVolume": Therock::safe_number(self, ticker.clone(), Value::from("askVolume"), Value::Undefined),
            "high": Therock::safe_number(self, ticker.clone(), Value::from("high"), Value::Undefined),
            "low": Therock::safe_number(self, ticker.clone(), Value::from("low"), Value::Undefined),
            ...
            "previousClose": Therock::safe_number(self, ticker.clone(), Value::from("previousClose"), Value::Undefined)
        }))).unwrap()));
    }
    ...
    fn market_ids(&mut self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Therock::market_id(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn market_symbols(&self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Therock::symbol(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn parse_bids_asks(&self, mut bidasks: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < bidasks.len() {
            result.push(Therock::parse_bid_ask(self, bidasks.get(i.into()), price_key.clone(), amount_key.clone()));
            i += 1;
        };
        return result.clone();
    }
    ...
    async fn fetch_l2_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut orderbook: Value = Therock::fetch_order_book(self, symbol.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_order_book(&self, mut orderbook: Value, mut symbol: Value, mut timestamp: Value, mut bids_key: Value, mut asks_key: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        ...
        price_key = price_key.or_default(Value::from(0));
        amount_key = amount_key.or_default(Value::from(1));
        let mut bids: Value = Therock::parse_bids_asks(self, self.safe_value(orderbook.clone(), bids_key.clone(), Value::new_array()), price_key.clone(), amount_key.clone());
        let mut asks: Value = Therock::parse_bids_asks(self, self.safe_value(orderbook.clone(), asks_key.clone(), Value::new_array()), price_key.clone(), amount_key.clone());
        return Value::Json(normalize(&Value::Json(json!({
            "symbol": symbol,
    ...
    fn parse_ohlcvs(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < ohlcvs.len() {
            results.push(Therock::parse_ohlcv(self, ohlcvs.get(i.into()), market.clone()));
            i += 1;
        };
    ...
    fn parse_positions(&self, mut positions: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        return Therock::filter_by_array(self, result.clone(), Value::from("symbol"), symbols.clone(), false.into());
    }
    ...
    fn parse_accounts(&self, mut accounts: Value, mut params: Value) -> Value {
        ...
        while i < accounts.len() {
            let mut account: Value = extend_2(Therock::parse_account(self, accounts.get(i.into())), params.clone());
    ...
    fn parse_trades(&mut self, mut trades: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < trades.len() {
            let mut trade: Value = extend_2(Therock::parse_trade(self, trades.get(i.into()), market.clone()), params.clone());
        ...
        let mut symbol: Value = if market.clone().is_nonnullish() { market.get(Value::from("symbol")) } else { Value::Undefined };
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Therock::filter_by_symbol_since_limit(self, result.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn parse_transactions(&self, mut transactions: Value, mut currency: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        while i < transactions.len() {
            let mut transaction: Value = extend_2(Therock::parse_transaction(self, transactions.get(i.into()), currency.clone()), params.clone());
        ...
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Therock::filter_by_currency_since_limit(self, result.clone(), code.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn parse_ledger(&self, mut data: Value, mut currency: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        while i < array_data.len() {
            let mut item_or_items: Value = Therock::parse_ledger_entry(self, array_data.get(i.into()), currency.clone());
    ...
    async fn fetch2(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        if self.get("enable_rate_limit".into()).is_truthy() {
            let mut cost: Value = Therock::calculate_rate_limiter_cost(self, api.clone(), method.clone(), path.clone(), params.clone(), config.clone(), context.clone());
            self.throttle(cost.clone()).await;
        };
        self.set("last_rest_request_timestamp".into(), self.milliseconds());
        let mut request: Value = Therock::sign(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone());
    ...
    async fn request(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        config = config.or_default(Value::new_object());
        context = context.or_default(Value::new_object());
        return Therock::fetch2(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone(), config.clone(), context.clone()).await;
    }
    ...
    async fn fetch_ohlcvc(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        Therock::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut trades: Value = Therock::fetch_trades(self, symbol.clone(), since.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_trading_view_ohlcv(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        timeframe = timeframe.or_default(Value::from("1m"));
        let mut result: Value = Therock::convert_trading_view_to_ohlcv(self, ohlcvs.clone(), Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined);
        return Therock::parse_ohlcvs(self, result.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    async fn edit_limit_buy_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_sell_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_order(&mut self, mut id: Value, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::edit_order(self, id.clone(), symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn edit_order(&mut self, mut id: Value, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Therock::cancel_order(self, id.clone(), symbol.clone(), Value::Undefined).await;
        return Therock::create_order(self, symbol.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    fn parse_bid_ask(&self, mut bidask: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        price_key = price_key.or_default(Value::from(0));
        amount_key = amount_key.or_default(Value::from(1));
        let mut price: Value = Therock::safe_number(self, bidask.clone(), price_key.clone(), Value::Undefined);
        let mut amount: Value = Therock::safe_number(self, bidask.clone(), amount_key.clone(), Value::Undefined);
    ...
    fn safe_currency(&self, mut currency_id: Value, mut currency: Value) -> Value {
        ...
        if currency_id.clone().is_nonnullish() {
            code = Therock::common_currency_code(self, currency_id.to_upper_case());
        };
        return Value::Json(normalize(&Value::Json(json!({
            "id": currency_id,
            "code": code
        ...
        if market_id.clone().is_nonnullish() {
            if self.get("markets_by_id".into()).is_nonnullish() && self.get("markets_by_id".into()).contains_key(market_id.clone()) {
                ...
                if parts_length.clone() == Value::from(2) {
                    ...
                    result.set("base".into(), Therock::safe_currency_code(self, result.get(Value::from("baseId")), Value::Undefined));
                    result.set("quote".into(), Therock::safe_currency_code(self, result.get(Value::from("quoteId")), Value::Undefined));
    ...
    async fn fetch_partial_balance(&mut self, mut part: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut balance: Value = Therock::fetch_balance(self, params.clone()).await;
        return balance.get(part.clone());
    }

    async fn fetch_free_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::fetch_partial_balance(self, Value::from("free"), params.clone()).await;
    }

    async fn fetch_used_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::fetch_partial_balance(self, Value::from("used"), params.clone()).await;
    }

    async fn fetch_total_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::fetch_partial_balance(self, Value::from("total"), params.clone()).await;
    }
    ...
    async fn fetch_funding_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Therock::fetch_transaction_fee(self, code.clone(), params.clone()).await;
    }
    ...
    async fn fetch_funding_fees(&mut self, mut codes: Value, mut params: Value) -> Value {
        ...
        return Therock::fetch_transaction_fees(self, codes.clone(), params.clone()).await;
    }
    ...
    async fn fetch_transaction_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Therock::fetch_transaction_fees(self, Value::Json(serde_json::Value::Array(vec![code.clone().into()])), params.clone()).await;
    }
    ...
    fn throw_broadly_matched_exception(&mut self, mut broad: Value, mut string: Value, mut message: Value) -> () {
        let mut broad_key: Value = Therock::find_broadly_matched_key(self, broad.clone(), string.clone());
    ...
    async fn fetch_order_status(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut order: Value = Therock::fetch_order(self, id.clone(), symbol.clone(), params.clone()).await;
        return order.get(Value::from("status"));
    }

    async fn fetch_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::fetch_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }

    async fn cancel_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::cancel_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }
    ...
    async fn create_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn create_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("limit"), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("limit"), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_market_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("market"), Value::from("buy"), amount.clone(), Value::Undefined, params.clone()).await;
    }

    async fn create_market_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Therock::create_order(self, symbol.clone(), Value::from("market"), Value::from("sell"), amount.clone(), Value::Undefined, params.clone()).await;
    }
    ...
    async fn create_post_only_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        return Therock::create_order(self, symbol.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Therock::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Therock::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), Value::Undefined, query.clone()).await;
    }

    fn safe_currency_code(&self, mut currency_id: Value, mut currency: Value) -> Value {
        currency = Therock::safe_currency(self, currency_id.clone(), currency.clone());
    ...
    fn parse_tickers(&self, mut tickers: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(tickers.clone()).is_truthy() {
            let mut i: usize = 0;
            while i < tickers.len() {
                let mut ticker: Value = extend_2(Therock::parse_ticker(self, tickers.get(i.into()), Value::Undefined), params.clone());
            ...
            while i < market_ids.len() {
                ...
                let mut market: Value = Therock::safe_market(self, market_id.clone(), Value::Undefined, Value::Undefined);
                let mut ticker: Value = extend_2(Therock::parse_ticker(self, tickers.get(market_id.clone()), market.clone()), params.clone());
                results.push(ticker.clone());
                i += 1;
            };
        };
        symbols = Therock::market_symbols(self, symbols.clone());
        return Therock::filter_by_array(self, results.clone(), Value::from("symbol"), symbols.clone(), Value::Undefined);
    }
    ...
    fn parse_deposit_addresses(&self, mut addresses: Value, mut codes: Value, mut indexed: Value, mut params: Value) -> Value {
        ...
        if codes.clone().is_nonnullish() {
            result = Therock::filter_by_array(self, result.clone(), Value::from("currency"), codes.clone(), false.into());
        };
    ...
    fn parse_funding_rate_histories(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        return Therock::filter_by_symbol_since_limit(self, sorted.clone(), symbol.clone(), since.clone(), limit.clone(), Value::Undefined);
    }

    fn safe_symbol(&self, mut market_id: Value, mut market: Value, mut delimiter: Value) -> Value {
        market = Therock::safe_market(self, market_id.clone(), market.clone(), delimiter.clone());
    ...
    fn parse_funding_rates(&self, mut response: Value, mut market: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < response.len() {
            let mut parsed: Value = Therock::parse_funding_rate(self, response.get(i.into()), market.clone());
    ...
    fn parse_open_interests(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        while i < response.len() {
            let mut entry: Value = response.get(i.into());
            let mut interest: Value = Therock::parse_open_interest(self, entry.clone(), market.clone());
    ...
    async fn fetch_mark_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if self.get("has".into()).get(Value::from("fetchMarkOHLCV")).is_truthy() {
            ...
            return Therock::fetch_ohlcv(self, symbol.clone(), timeframe.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
        } else {
    ...
    fn parse_account(&self, mut account: Value) -> Value {
        ...
        if accounts_by_type.contains_key(account.clone()) {
            ...
            let mut market: Value = Therock::market(self, account.clone());
            return market.get(Value::from("id"));
        } else {
    ...
    async fn load_markets_helper(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if !reload.is_truthy() && self.get("markets".into()).is_truthy() {
            if !self.get("markets_by_id".into()).is_truthy() {
                return Therock::set_markets(self, self.get("markets".into()), Value::Undefined);
            };
            return self.get("markets".into());
        };
        ...
        let mut markets: Value = Therock::fetch_markets(self, params.clone()).await;
        return Therock::set_markets(self, markets.clone(), currencies.clone());
    }
    ...
    async fn load_markets(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if reload.is_truthy() && !self.get("reloading_markets".into()).is_truthy() || !self.get("markets_loading".into()).is_truthy() {
            ...
            let mut markets_loading: Value = Therock::load_markets_helper(self, reload.clone(), params.clone()).await;
    ...
    async fn dispatch(&mut self, method: Value, params: Value, context: Value) -> Value {
        match method {
            Value::Json(serde_json::Value::String(ref m)) => {
                match m.as_ref() {
                    "publicGetFunds" => Therock::request(self, "funds".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsId" => Therock::request(self, "funds/{id}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsIdOrderbook" => Therock::request(self, "funds/{id}/orderbook".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsIdTicker" => Therock::request(self, "funds/{id}/ticker".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsIdTrades" => Therock::request(self, "funds/{id}/trades".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsIdOhlcStatistics" => Therock::request(self, "funds/{id}/ohlc_statistics".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetFundsTickers" => Therock::request(self, "funds/tickers".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetBalances" => Therock::request(self, "balances".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetBalancesId" => Therock::request(self, "balances/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetDiscounts" => Therock::request(self, "discounts".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetDiscountsId" => Therock::request(self, "discounts/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFunds" => Therock::request(self, "funds".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsId" => Therock::request(self, "funds/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsIdTrades" => Therock::request(self, "funds/{id}/trades".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsFundIdOrders" => Therock::request(self, "funds/{fund_id}/orders".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsFundIdOrdersId" => Therock::request(self, "funds/{fund_id}/orders/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsFundIdPositionBalances" => Therock::request(self, "funds/{fund_id}/position_balances".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsFundIdPositions" => Therock::request(self, "funds/{fund_id}/positions".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetFundsFundIdPositionsId" => Therock::request(self, "funds/{fund_id}/positions/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetTransactions" => Therock::request(self, "transactions".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetTransactionsId" => Therock::request(self, "transactions/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetWithdrawLimitsId" => Therock::request(self, "withdraw_limits/{id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetWithdrawLimits" => Therock::request(self, "withdraw_limits".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostAtmsWithdraw" => Therock::request(self, "atms/withdraw".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostFundsFundIdOrders" => Therock::request(self, "funds/{fund_id}/orders".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateDeleteFundsFundIdOrdersId" => Therock::request(self, "funds/{fund_id}/orders/{id}".into(), "private".into(), "DELETE".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateDeleteFundsFundIdOrdersRemoveAll" => Therock::request(self, "funds/{fund_id}/orders/remove_all".into(), "private".into(), "DELETE".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    _ => unimplemented!(),
                }
            },
            _ => unimplemented!()
...
pub struct TherockImpl(Value);
impl Exchange for TherockImpl {}
impl Therock for TherockImpl {}
impl ValueTrait for TherockImpl {
...
impl TherockImpl {
    pub fn new(params: Value) -> Self {
        let mut rv = TherockImpl(match params {
            Value::Json(_) => params,
            _ => Value::new_object()
        });
        ExchangeImpl::init(&mut rv.0);

        let config_entries = Therock::describe(&rv);
