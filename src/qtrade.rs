...
pub trait Qtrade : Exchange {
    ...
    async fn fetch_markets(&mut self, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "publicGetMarkets".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "markets":[
        ...
        let mut markets: Value = self.safe_value(data.clone(), Value::from("markets"), Value::new_array());
        let mut result: Value = Value::new_array();
        let mut i: usize = 0;
        while i < markets.len() {
            let mut market: Value = markets.get(i.into());
            let mut market_id: Value = self.safe_string(market.clone(), Value::from("market_string"), Value::Undefined);
            let mut numeric_id: Value = self.safe_integer(market.clone(), Value::from("id"), Value::Undefined);
            let mut base_id: Value = self.safe_string(market.clone(), Value::from("market_currency"), Value::Undefined);
            let mut quote_id: Value = self.safe_string(market.clone(), Value::from("base_currency"), Value::Undefined);
            let mut base: Value = Qtrade::safe_currency_code(self, base_id.clone(), Value::Undefined);
            let mut quote: Value = Qtrade::safe_currency_code(self, quote_id.clone(), Value::Undefined);
            let mut can_view: Value = self.safe_value(market.clone(), Value::from("can_view"), false.into());
            let mut can_trade: Value = self.safe_value(market.clone(), Value::from("can_trade"), false.into());
            let mut active: Value = (can_trade.is_truthy() && can_view.is_truthy()).into();
            result.push(Value::Json(normalize(&Value::Json(json!({
                ...
                "taker": Qtrade::safe_number(self, market.clone(), Value::from("taker_fee"), Value::Undefined),
                "maker": Qtrade::safe_number(self, market.clone(), Value::from("maker_fee"), Value::Undefined),
                ...
                "precision": Value::Json(normalize(&Value::Json(json!({
                    "amount": self.parse_number(Qtrade::parse_precision(self, self.safe_string(market.clone(), Value::from("market_precision"), Value::Undefined)), Value::Undefined),
                    "price": self.parse_number(Qtrade::parse_precision(self, self.safe_string(market.clone(), Value::from("base_precision"), Value::Undefined)), Value::Undefined)
                }))).unwrap()),
                "limits": Value::Json(normalize(&Value::Json(json!({
                    ...
                    "amount": Value::Json(normalize(&Value::Json(json!({
                        "min": Qtrade::safe_number(self, market.clone(), Value::from("minimum_sell_value"), Value::Undefined),
                        "max": Value::Undefined
                    }))).unwrap()),
                    ...
                    "cost": Value::Json(normalize(&Value::Json(json!({
                        "min": Qtrade::safe_number(self, market.clone(), Value::from("minimum_buy_value"), Value::Undefined),
                        "max": Value::Undefined
                    }))).unwrap())
                }))).unwrap()),
                "info": market
    ...
    /// # Arguments
    ///
    /// * `params` {object} - extra parameters specific to the qtrade api endpoint
    async fn fetch_currencies(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut response: Value = Qtrade::dispatch(self, "publicGetCurrencies".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "currencies":[
        ...
        while i < currencies.len() {
            ...
            let mut code: Value = Qtrade::safe_currency_code(self, id.clone(), Value::Undefined);
            let mut name: Value = self.safe_string(currency.clone(), Value::from("long_name"), Value::Undefined);
            let mut r#type: Value = self.safe_string(currency.clone(), Value::from("type"), Value::Undefined);
            let mut can_withdraw: Value = self.safe_value(currency.clone(), Value::from("can_withdraw"), true.into());
            let mut withdraw_disabled: Value = self.safe_value(currency.clone(), Value::from("withdraw_disabled"), false.into());
            let mut deposit_disabled: Value = self.safe_value(currency.clone(), Value::from("deposit_disabled"), false.into());
            let mut deposit: Value = (!deposit_disabled.is_truthy()).into();
            let mut withdraw: Value = (can_withdraw.is_truthy() && !withdraw_disabled.is_truthy()).into();
            let mut config: Value = self.safe_value(currency.clone(), Value::from("config"), Value::new_object());
            let mut status: Value = self.safe_string(currency.clone(), Value::from("status"), Value::Undefined);
            let mut active: Value = (withdraw.is_truthy() && deposit.is_truthy() && status.clone() == Value::from("ok")).into();
            result.set(code.clone(), Value::Json(normalize(&Value::Json(json!({
                ...
                "type": r#type,
                "name": name,
                "fee": Qtrade::safe_number(self, config.clone(), Value::from("withdraw_fee"), Value::Undefined),
                "precision": self.parse_number(Qtrade::parse_precision(self, self.safe_string(currency.clone(), Value::from("precision"), Value::Undefined)), Value::Undefined),
                "active": active,
                "deposit": deposit,
                "withdraw": withdraw,
                "limits": Value::Json(normalize(&Value::Json(json!({
                    "amount": Value::Json(normalize(&Value::Json(json!({
                        "min": Qtrade::safe_number(self, currency.clone(), Value::from("minimum_order"), Value::Undefined),
                        "max": Value::Undefined
                    }))).unwrap()),
    ...
    fn parse_ohlcv(&self, mut ohlcv: Value, mut market: Value) -> Value {
        ...
        //     }
        //
        return Value::Json(serde_json::Value::Array(vec![self.parse8601(self.safe_string(ohlcv.clone(), Value::from("time"), Value::Undefined)).into(), Qtrade::safe_number(self, ohlcv.clone(), Value::from("open"), Value::Undefined).into(), Qtrade::safe_number(self, ohlcv.clone(), Value::from("high"), Value::Undefined).into(), Qtrade::safe_number(self, ohlcv.clone(), Value::from("low"), Value::Undefined).into(), Qtrade::safe_number(self, ohlcv.clone(), Value::from("close"), Value::Undefined).into(), Qtrade::safe_number(self, ohlcv.clone(), Value::from("market_volume"), Value::Undefined).into()]));
    }
    ...
    async fn fetch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        params = params.or_default(Value::new_object());
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut market: Value = Qtrade::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "market_string": market.get(Value::from("id")),
            "interval": self.get("timeframes".into()).get(timeframe.clone())
        }))).unwrap());
        let mut response: Value = Qtrade::dispatch(self, "publicGetMarketMarketStringOhlcvInterval".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "slices":[
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut ohlcvs: Value = self.safe_value(data.clone(), Value::from("slices"), Value::new_array());
        return Qtrade::parse_ohlcvs(self, ohlcvs.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    async fn fetch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "publicGetOrderbookMarketString".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "buy":{
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut orderbook: Value = Value::new_object();
        let mut sides: Value = Value::Json(normalize(&Value::Json(json!({
            "buy": "bids",
            "sell": "asks"
        }))).unwrap());
        let mut keys: Value = Object::keys(sides.clone());
        let mut i: usize = 0;
        while i < keys.len() {
            let mut key: Value = keys.get(i.into());
            let mut side: Value = sides.get(key.clone());
            let mut bidasks: Value = self.safe_value(data.clone(), key.clone(), Value::new_object());
            let mut prices: Value = Object::keys(bidasks.clone());
            let mut result: Value = Value::new_array();
            let mut j: usize = 0;
            while j < prices.len() {
                let mut price_as_string: Value = prices.get(j.into());
                let mut price: Value = Qtrade::safe_number(self, prices.clone(), Value::from(j), Value::Undefined);
                let mut amount: Value = Qtrade::safe_number(self, bidasks.clone(), price_as_string.clone(), Value::Undefined);
                result.push(Value::Json(serde_json::Value::Array(vec![price.clone().into(), amount.clone().into()])));
                j += 1;
            };
            orderbook.set(side.clone(), result.clone());
            i += 1;
        };
        let mut timestamp: Value = self.safe_integer_product(data.clone(), Value::from("last_change"), Value::from(0.001), Value::Undefined);
        return Qtrade::parse_order_book(self, orderbook.clone(), market.get(Value::from("symbol")), timestamp.clone(), Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined);
    }
    ...
    fn parse_ticker(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        //     }
        //
        let mut market_id: Value = self.safe_string(ticker.clone(), Value::from("id_hr"), Value::Undefined);
        let mut symbol: Value = Qtrade::safe_symbol(self, market_id.clone(), market.clone(), Value::from("_"));
        let mut timestamp: Value = self.safe_integer_product(ticker.clone(), Value::from("last_change"), Value::from(0.001), Value::Undefined);
        let mut previous: Value = self.safe_string(ticker.clone(), Value::from("day_open"), Value::Undefined);
        let mut last: Value = self.safe_string(ticker.clone(), Value::from("last"), Value::Undefined);
        let mut day_change: Value = self.safe_string(ticker.clone(), Value::from("day_change"), Value::Undefined);
        let mut average: Value = self.safe_string(ticker.clone(), Value::from("day_avg_price"), Value::Undefined);
        let mut base_volume: Value = self.safe_string(ticker.clone(), Value::from("day_volume_market"), Value::Undefined);
        let mut quote_volume: Value = self.safe_string(ticker.clone(), Value::from("day_volume_base"), Value::Undefined);
        let mut percentage: Value = Precise::string_mul(day_change.clone(), Value::from("100"));
        let mut change: Value = Precise::string_mul(day_change.clone(), previous.clone());
        return Qtrade::safe_ticker(self, Value::Json(normalize(&Value::Json(json!({
            "symbol": symbol,
            "timestamp": timestamp,
            "datetime": self.iso8601(timestamp.clone()),
            "high": self.safe_string(ticker.clone(), Value::from("day_high"), Value::Undefined),
            "low": self.safe_string(ticker.clone(), Value::from("day_low"), Value::Undefined),
    ...
    async fn fetch_tickers(&mut self, mut symbols: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        symbols = Qtrade::market_symbols(self, symbols.clone());
        let mut response: Value = Qtrade::dispatch(self, "publicGetTickers".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "markets":[
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut tickers: Value = self.safe_value(data.clone(), Value::from("markets"), Value::new_array());
        let mut result: Value = Value::new_object();
        let mut i: usize = 0;
        while i < tickers.len() {
            let mut ticker: Value = Qtrade::parse_ticker(self, tickers.get(i.into()), Value::Undefined);
            let mut symbol: Value = ticker.get(Value::from("symbol"));
            result.set(symbol.clone(), ticker.clone());
            i += 1;
        };
        return Qtrade::filter_by_array(self, result.clone(), Value::from("symbol"), symbols.clone(), Value::Undefined);
    }
    ...
    async fn fetch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "publicGetTickerMarketString".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "ask":"0.02423119",
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        return Qtrade::parse_ticker(self, data.clone(), market.clone());
    }
    ...
    async fn fetch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "publicGetMarketMarketStringTrades".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "trades":[
        ...
        return Qtrade::parse_trades(self, trades.clone(), market.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_my_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        // Returns newest trades first when true
        // 'older_than': 123, // returns trades with id < older_than
        // 'newer_than': 123, // returns trades with id > newer_than
        let mut market: Value = Value::Undefined;
        let mut numeric_id: Value = self.safe_value(params.clone(), Value::from("market_id"), Value::Undefined);
        if numeric_id.clone().is_nonnullish() {
            request.set("market_id".into(), numeric_id.clone());
        } else if symbol.clone().is_nonnullish() {
            // mutually exclusive with market_string
            market = Qtrade::market(self, symbol.clone());
            request.set("market_string".into(), market.get(Value::from("id")));
        };
        let mut response: Value = Qtrade::dispatch(self, "privateGetTrades".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "trades":[
    ...
    fn parse_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        let mut timestamp: Value = self.safe_integer_product(trade.clone(), Value::from("created_at_ts"), Value::from(0.001), Value::Undefined);
        ...
        let mut market_id: Value = self.safe_string(trade.clone(), Value::from("market_string"), Value::Undefined);
        market = Qtrade::safe_market(self, market_id.clone(), market.clone(), Value::Undefined);
        let mut cost: Value = self.safe_string_2(trade.clone(), Value::from("base_volume"), Value::from("base_amount"), Value::Undefined);
        let mut price: Value = self.safe_string(trade.clone(), Value::from("price"), Value::Undefined);
        let mut amount: Value = self.safe_string_2(trade.clone(), Value::from("market_amount"), Value::from("amount"), Value::Undefined);
        let mut fee: Value = Value::Undefined;
        let mut fee_cost: Value = self.safe_string(trade.clone(), Value::from("base_fee"), Value::Undefined);
        if fee_cost.clone().is_nonnullish() {
            let mut fee_currency_code: Value = if market.clone().is_nullish() { Value::Undefined } else { market.get(Value::from("quote")) };
            fee = Value::Json(normalize(&Value::Json(json!({
                "currency": fee_currency_code,
                "cost": fee_cost
            }))).unwrap());
        };
        let mut taker: Value = self.safe_value(trade.clone(), Value::from("taker"), true.into());
        let mut taker_or_maker: Value = if taker.is_truthy() { Value::from("taker") } else { Value::from("maker") };
        let mut order_id: Value = self.safe_string(trade.clone(), Value::from("order_id"), Value::Undefined);
        return Qtrade::safe_trade(self, Value::Json(normalize(&Value::Json(json!({
            "id": id,
            "info": trade,
            "timestamp": timestamp,
            "datetime": self.iso8601(timestamp.clone()),
    ...
    async fn fetch_trading_fee(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "publicGetMarketMarketString".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         data: {
        //             market: {
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut market_data: Value = self.safe_value(data.clone(), Value::from("market"), Value::new_object());
        return Value::Json(normalize(&Value::Json(json!({
            "info": market_data,
            "symbol": market.get(Value::from("symbol")),
            "maker": Qtrade::safe_number(self, market_data.clone(), Value::from("maker_fee"), Value::Undefined),
            "taker": Qtrade::safe_number(self, market_data.clone(), Value::from("taker_fee"), Value::Undefined),
            "percentage": true,
            "tierBased": true
        }))).unwrap());
    }
    ...
    fn parse_balance(&self, mut response: Value) -> Value {
        ...
        while i < balances.len() {
            ...
            let mut code: Value = Qtrade::safe_currency_code(self, currency_id.clone(), Value::Undefined);
            let mut account: Value = if result.contains_key(code.clone()) { result.get(code.clone()) } else { Qtrade::account(self) };
            account.set("free".into(), self.safe_string(balance.clone(), Value::from("balance"), Value::Undefined));
            account.set("used".into(), Value::from("0"));
            result.set(code.clone(), account.clone());
            i += 1;
        };
        balances = self.safe_value(data.clone(), Value::from("order_balances"), Value::new_array());
        let mut i: usize = 0;
        while i < balances.len() {
            let mut balance: Value = balances.get(i.into());
            let mut currency_id: Value = self.safe_string(balance.clone(), Value::from("currency"), Value::Undefined);
        ...
        return Qtrade::safe_balance(self, result.clone());
    }
    ...
    async fn fetch_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut response: Value = Qtrade::dispatch(self, "privateGetBalancesAll".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "balances": [
        ...
        //     }
        //
        return Qtrade::parse_balance(self, response.clone());
    }
    ...
    async fn create_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        if r#type.clone() != Value::from("limit") {
            panic!(r###"InvalidOrder::new(self.get("id".into()) + Value::from(" createOrder() allows limit orders only"))"###);
        };
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut market: Value = Qtrade::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "amount": Qtrade::amount_to_precision(self, market.get(Value::from("symbol")), amount.clone()),
            "market_id": market.get(Value::from("numericId")),
            "price": Qtrade::price_to_precision(self, market.get(Value::from("symbol")), price.clone())
        }))).unwrap());
        let mut method: Value = if side.clone() == Value::from("sell") { Value::from("privatePostSellLimit") } else { Value::from("privatePostBuyLimit") };
        let mut response: Value = Qtrade::dispatch(self, method, extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data": {
        //             "order": {
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut order: Value = self.safe_value(data.clone(), Value::from("order"), Value::new_object());
        return Qtrade::parse_order(self, order.clone(), market.clone());
    }
    ...
    fn parse_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        let mut side_type: Value = self.safe_string(order.clone(), Value::from("order_type"), Value::Undefined);
        let mut order_type: Value = Value::Undefined;
        let mut side: Value = Value::Undefined;
        if side_type.clone().is_nonnullish() {
            let mut parts: Value = side_type.split(Value::from("_"));
            side = self.safe_string(parts.clone(), Value::from(0), Value::Undefined);
            order_type = self.safe_string(parts.clone(), Value::from(1), Value::Undefined);
        };
        let mut price: Value = self.safe_string(order.clone(), Value::from("price"), Value::Undefined);
        let mut amount: Value = self.safe_string(order.clone(), Value::from("market_amount"), Value::Undefined);
        let mut remaining: Value = self.safe_string(order.clone(), Value::from("market_amount_remaining"), Value::Undefined);
        let mut open: Value = self.safe_value(order.clone(), Value::from("open"), false.into());
        let mut close_reason: Value = self.safe_string(order.clone(), Value::from("close_reason"), Value::Undefined);
        let mut status: Value = Value::Undefined;
        if open.is_truthy() {
            status = Value::from("open");
        } else if close_reason.clone() == Value::from("canceled") {
            status = Value::from("canceled");
        } else {
            status = Value::from("closed");
        };
        let mut market_id: Value = self.safe_string(order.clone(), Value::from("market_string"), Value::Undefined);
        market = Qtrade::safe_market(self, market_id.clone(), market.clone(), Value::from("_"));
        let mut symbol: Value = market.get(Value::from("symbol"));
        let mut raw_trades: Value = self.safe_value(order.clone(), Value::from("trades"), Value::new_array());
        return Qtrade::safe_order(self, Value::Json(normalize(&Value::Json(json!({
            "info": order,
            "id": id,
    ...
    ///
    /// * `id` {string} - order id
    /// * `symbol` {string|undefined} - not used by qtrade cancelOrder ()
    /// * `params` {object} - extra parameters specific to the qtrade api endpoint
    async fn cancel_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        return Qtrade::dispatch(self, "privatePostCancelOrder".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
    }
    ...
    /// # Arguments
    ///
    /// * `symbol` {string|undefined} - not used by qtrade fetchOrder
    /// * `params` {object} - extra parameters specific to the qtrade api endpoint
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetOrderOrderId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "order":{
        ...
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut order: Value = self.safe_value(data.clone(), Value::from("order"), Value::new_object());
        return Qtrade::parse_order(self, order.clone(), Value::Undefined);
    }
    ...
    async fn fetch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetOrders".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "orders":[
        ...
        return Qtrade::parse_orders(self, orders.clone(), market.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_open_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return Qtrade::fetch_orders(self, symbol.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
    }
    ...
    async fn fetch_deposit_address(&mut self, mut code: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut currency: Value = Qtrade::currency(self, code.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "currency": currency.get(Value::from("id"))
        }))).unwrap());
        let mut response: Value = Qtrade::dispatch(self, "privatePostDepositAddressCurrency".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "address":"0xe0cd26f9A60118555247aE6769A5d241D91f07f2",
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        return Qtrade::parse_deposit_address(self, data.clone(), currency.clone());
    }
    ...
    /// * `code` {string|undefined} - not used by qtrade fetchDeposit ()
    /// * `params` {object} - extra parameters specific to the qtrade api endpoint
    async fn fetch_deposit(&mut self, mut id: Value, mut code: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetDepositDepositId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "deposit":{
        ...
        return Qtrade::parse_transaction(self, deposit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_deposits(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if code.clone().is_nonnullish() {
            currency = Qtrade::currency(self, code.clone());
        };
        let mut response: Value = Qtrade::dispatch(self, "privateGetDeposits".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data":{
        //             "deposits":[
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut deposits: Value = self.safe_value(data.clone(), Value::from("deposits"), Value::new_array());
        return Qtrade::parse_transactions(self, deposits.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    async fn fetch_transfers(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetTransfers".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "data": {
        //             "transfers": [
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut transfers: Value = self.safe_value(data.clone(), Value::from("transfers"), Value::new_array());
        return Qtrade::parse_transfers(self, transfers.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    fn parse_transfer(&self, mut transfer: Value, mut currency: Value) -> Value {
        ...
        let mut date_time: Value = self.safe_string(transfer.clone(), Value::from("created_at"), Value::Undefined);
        return Value::Json(normalize(&Value::Json(json!({
            "info": transfer,
            "id": self.safe_string(transfer.clone(), Value::from("id"), Value::Undefined),
            "timestamp": self.parse8601(date_time.clone()),
            "datetime": date_time,
            "currency": Qtrade::safe_currency_code(self, currency_id.clone(), currency.clone()),
            "amount": Qtrade::safe_number(self, transfer.clone(), Value::from("amount"), Value::Undefined),
            "fromAccount": self.safe_string(transfer.clone(), Value::from("sender_id"), Value::Undefined),
    ...
    /// * `code` {string|undefined} - not used by qtrade.fetchWithdrawal
    /// * `params` {object} - extra parameters specific to the qtrade api endpoint
    async fn fetch_withdrawal(&mut self, mut id: Value, mut code: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetWithdrawWithdrawId".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         data: {
        //             withdraw: {
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut withdrawal: Value = self.safe_value(data.clone(), Value::from("withdraw"), Value::new_object());
        return Qtrade::parse_transaction(self, withdrawal.clone(), Value::Undefined);
    }
    ...
    async fn fetch_withdrawals(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Qtrade::dispatch(self, "privateGetWithdraws".into(), params.clone(), Value::Undefined).await;
        //     {
        //         "data":{
        //             "withdraws":[
        //                 {
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut withdrawals: Value = self.safe_value(data.clone(), Value::from("withdraws"), Value::new_array());
        return Qtrade::parse_transactions(self, withdrawals.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined);
    }
    ...
    fn parse_transaction(&self, mut transaction: Value, mut currency: Value) -> Value {
        ...
        let mut network_data: Value = self.safe_value(transaction.clone(), Value::from("network_data"), Value::new_object());
        let mut unsigned_tx: Value = self.safe_value(network_data.clone(), Value::from("unsigned_tx"), Value::new_object());
        let mut address_from: Value = self.safe_string(unsigned_tx.clone(), Value::from("from"), Value::Undefined);
        let mut txid: Value = self.safe_string(network_data.clone(), Value::from("txid"), Value::Undefined);
        ...
        let mut cancel_requested: Value = self.safe_value(transaction.clone(), Value::from("cancel_requested"), Value::Undefined);
        let mut r#type: Value = if cancel_requested.clone().is_nullish() { Value::from("deposit") } else { Value::from("withdrawal") };
        let mut amount: Value = Qtrade::safe_number(self, transaction.clone(), Value::from("amount"), Value::Undefined);
        let mut currency_id: Value = self.safe_string(transaction.clone(), Value::from("currency"), Value::Undefined);
        let mut code: Value = Qtrade::safe_currency_code(self, currency_id.clone(), Value::Undefined);
        let mut status: Value = Qtrade::parse_transaction_status(self, self.safe_string(transaction.clone(), Value::from("status"), Value::Undefined));
        let mut status_code: Value = self.safe_string(transaction.clone(), Value::from("code"), Value::Undefined);
        if cancel_requested.is_truthy() {
            status = Value::from("canceled");
        } else if status.clone().is_nullish() {
            status = Qtrade::parse_transaction_status(self, status_code.clone());
        };
        let mut fee: Value = Value::Undefined;
        return Value::Json(normalize(&Value::Json(json!({
            "info": transaction,
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        (tag, params) = shift_2(Qtrade::handle_withdraw_tag_and_params(self, tag.clone(), params.clone()));
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut currency: Value = Qtrade::currency(self, code.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "address": address,
        ...
        if tag.clone().is_nonnullish() {
            request.set("address".into(), Value::from(":") + tag.clone());
        };
        let mut response: Value = Qtrade::dispatch(self, "privatePostWithdraw".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "data": {
        //             "code": "initiated",
        ...
        //
        let mut data: Value = self.safe_value(response.clone(), Value::from("data"), Value::new_object());
        let mut result: Value = Qtrade::parse_transaction(self, data.clone(), Value::Undefined);
    ...
    fn sign(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value) -> Value {
        ...
        params = params.or_default(Value::new_object());
        let mut url: Value = Value::from("/") + self.get("version".into()) + Value::from("/");
        if api.clone() == Value::from("private") {
            url = url +  Value::from("user/");
        };
        ...
        if method.clone() == Value::from("POST") {
            body = self.json(request.clone(), Value::Undefined);
        } else {
            if Object::keys(request.clone()).len() > 0 {
                url = url +  Value::from("?") + self.urlencode(request.clone());
            };
        };
        if api.clone() == Value::from("private") {
            let mut timestamp: Value = self.milliseconds().to_string();
            let mut body_as_string: Value = if method.clone() == Value::from("POST") { body.clone() } else { Value::from("") };
            let mut auth: Value = Value::Json(serde_json::Value::Array(vec![method.clone().into(), url.clone().into(), timestamp.clone().into(), body_as_string.clone().into(), self.get("secret".into()).into()])).join(Value::from("
"));
            // eslint-disable-line quotes
            let mut hash: Value = self.hash(self.encode(auth.clone()), Value::from("sha256"), Value::from("base64"));
            let mut key: Value = self.get("apiKey".into());
            if key.typeof_() != Value::from("string") {
                key = key.to_string();
            };
            let mut signature: Value = Value::from("HMAC-SHA256 ") + key.clone() + Value::from(":") + hash.clone();
    ...
    fn handle_errors(&mut self, mut code: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut request_headers: Value, mut request_body: Value) -> Value {
        ...
        let mut errors: Value = self.safe_value(response.clone(), Value::from("errors"), Value::new_array());
        let mut num_errors: Value = errors.len().into();
        if num_errors.clone() < Value::from(1) {
            return Value::Undefined;
        };
        ...
        while i < errors.len() {
            ...
            Qtrade::throw_exactly_matched_exception(self, self.get("exceptions".into()).get(Value::from("exact")), error_code.clone(), feedback.clone());
            i += 1;
        };
    ...
    fn safe_ledger_entry(&self, mut entry: Value, mut currency: Value) -> Value {
        currency = Qtrade::safe_currency(self, Value::Undefined, currency.clone());
        ...
        if fee.clone().is_nonnullish() {
            fee.set("cost".into(), Qtrade::safe_number(self, fee.clone(), Value::from("cost"), Value::Undefined));
        };
    ...
    fn set_markets(&mut self, mut markets: Value, mut currencies: Value) -> Value {
        ...
        while i < market_values.len() {
            let mut market: Value = self.deep_extend_4(Qtrade::safe_market(self, Value::Undefined, Value::Undefined, Value::Undefined), Value::Json(normalize(&Value::Json(json!({
    ...
    fn safe_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        if parse_filled.is_truthy() || parse_cost.is_truthy() || should_parse_fees.is_truthy() {
            ...
            trades = Qtrade::parse_trades(self, raw_trades.clone(), market.clone(), Value::Undefined, Value::Undefined, Value::Json(normalize(&Value::Json(json!({
        ...
        if should_parse_fees.is_truthy() {
            let mut reduced_fees: Value = if self.get("reduce_fees".into()).is_truthy() { Qtrade::reduce_fees_by_currency(self, fees.clone()) } else { fees.clone() };
            let mut reduced_length: Value = reduced_fees.len().into();
            let mut i: usize = 0;
            while i < reduced_length.clone().into() {
                reduced_fees.get(i.into()).set("cost".into(), Qtrade::safe_number(self, reduced_fees.get(i.into()), Value::from("cost"), Value::Undefined));
                if reduced_fees.get(i.into()).contains_key(Value::from("rate")) {
                    reduced_fees.get(i.into()).set("rate".into(), Qtrade::safe_number(self, reduced_fees.get(i.into()), Value::from("rate"), Value::Undefined));
                };
                i += 1;
            };
            if !parse_fee.is_truthy() && reduced_length.clone() == Value::from(0) {
                ...
                if fee.contains_key(Value::from("rate")) {
                    fee.set("rate".into(), Qtrade::safe_number(self, fee.clone(), Value::from("rate"), Value::Undefined));
                };
        ...
        while i < trades.len() {
            ...
            entry.set("amount".into(), Qtrade::safe_number(self, entry.clone(), Value::from("amount"), Value::Undefined));
            entry.set("price".into(), Qtrade::safe_number(self, entry.clone(), Value::from("price"), Value::Undefined));
            entry.set("cost".into(), Qtrade::safe_number(self, entry.clone(), Value::from("cost"), Value::Undefined));
    ...
    fn parse_orders(&mut self, mut orders: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(orders.clone()).is_truthy() {
            let mut i: usize = 0;
            while i < orders.len() {
                let mut order: Value = extend_2(Qtrade::parse_order(self, orders.get(i.into()), market.clone()), params.clone());
            ...
            while i < ids.len() {
                ...
                let mut order: Value = extend_2(Qtrade::parse_order(self, extend_2(Value::Json(normalize(&Value::Json(json!({
                    "id": id
        ...
        return Qtrade::filter_by_symbol_since_limit(self, results.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn safe_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        if should_parse_fees.is_truthy() {
            ...
            let mut trade_fee: Value = self.safe_value(trade.clone(), Value::from("fee"), Value::Undefined);
            if trade_fee.clone().is_nonnullish() {
                trade_fee.set("cost".into(), Qtrade::safe_number(self, trade_fee.clone(), Value::from("cost"), Value::Undefined));
                if trade_fee.contains_key(Value::from("rate")) {
                    trade_fee.set("rate".into(), Qtrade::safe_number(self, trade_fee.clone(), Value::from("rate"), Value::Undefined));
                };
    ...
    fn safe_ticker(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        return extend_2(ticker.clone(), Value::Json(normalize(&Value::Json(json!({
            "bid": Qtrade::safe_number(self, ticker.clone(), Value::from("bid"), Value::Undefined),
            "bidVolume": Qtrade::safe_number(self, ticker.clone(), Value::from("bidVolume"), Value::Undefined),
            "ask": Qtrade::safe_number(self, ticker.clone(), Value::from("ask"), Value::Undefined),
            "askVolume": Qtrade::safe_number(self, ticker.clone(), Value::from("askVolume"), Value::Undefined),
            "high": Qtrade::safe_number(self, ticker.clone(), Value::from("high"), Value::Undefined),
            "low": Qtrade::safe_number(self, ticker.clone(), Value::from("low"), Value::Undefined),
            ...
            "previousClose": Qtrade::safe_number(self, ticker.clone(), Value::from("previousClose"), Value::Undefined)
        }))).unwrap()));
    }
    ...
    fn market_ids(&mut self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Qtrade::market_id(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn market_symbols(&self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Qtrade::symbol(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn parse_bids_asks(&self, mut bidasks: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < bidasks.len() {
            result.push(Qtrade::parse_bid_ask(self, bidasks.get(i.into()), price_key.clone(), amount_key.clone()));
            i += 1;
        };
        return result.clone();
    }
    ...
    async fn fetch_l2_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut orderbook: Value = Qtrade::fetch_order_book(self, symbol.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_order_book(&self, mut orderbook: Value, mut symbol: Value, mut timestamp: Value, mut bids_key: Value, mut asks_key: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        ...
        price_key = price_key.or_default(Value::from(0));
        amount_key = amount_key.or_default(Value::from(1));
        let mut bids: Value = Qtrade::parse_bids_asks(self, self.safe_value(orderbook.clone(), bids_key.clone(), Value::new_array()), price_key.clone(), amount_key.clone());
        let mut asks: Value = Qtrade::parse_bids_asks(self, self.safe_value(orderbook.clone(), asks_key.clone(), Value::new_array()), price_key.clone(), amount_key.clone());
        return Value::Json(normalize(&Value::Json(json!({
            "symbol": symbol,
    ...
    fn parse_ohlcvs(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < ohlcvs.len() {
            results.push(Qtrade::parse_ohlcv(self, ohlcvs.get(i.into()), market.clone()));
            i += 1;
        };
    ...
    fn parse_leverage_tiers(&self, mut response: Value, mut symbols: Value, mut market_id_key: Value) -> Value {
        ...
        while i < response.len() {
            ...
            let mut market: Value = Qtrade::safe_market(self, id.clone(), Value::Undefined, Value::Undefined);
            let mut symbol: Value = market.get(Value::from("symbol"));
    ...
    fn parse_positions(&self, mut positions: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        return Qtrade::filter_by_array(self, result.clone(), Value::from("symbol"), symbols.clone(), false.into());
    }
    ...
    fn parse_accounts(&self, mut accounts: Value, mut params: Value) -> Value {
        ...
        while i < accounts.len() {
            let mut account: Value = extend_2(Qtrade::parse_account(self, accounts.get(i.into())), params.clone());
    ...
    fn parse_trades(&mut self, mut trades: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < trades.len() {
            let mut trade: Value = extend_2(Qtrade::parse_trade(self, trades.get(i.into()), market.clone()), params.clone());
        ...
        let mut symbol: Value = if market.clone().is_nonnullish() { market.get(Value::from("symbol")) } else { Value::Undefined };
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Qtrade::filter_by_symbol_since_limit(self, result.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn parse_transactions(&self, mut transactions: Value, mut currency: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        while i < transactions.len() {
            let mut transaction: Value = extend_2(Qtrade::parse_transaction(self, transactions.get(i.into()), currency.clone()), params.clone());
        ...
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Qtrade::filter_by_currency_since_limit(self, result.clone(), code.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn parse_transfers(&self, mut transfers: Value, mut currency: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        while i < transfers.len() {
            let mut transfer: Value = extend_2(Qtrade::parse_transfer(self, transfers.get(i.into()), currency.clone()), params.clone());
    ...
    async fn fetch2(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        if self.get("enable_rate_limit".into()).is_truthy() {
            let mut cost: Value = Qtrade::calculate_rate_limiter_cost(self, api.clone(), method.clone(), path.clone(), params.clone(), config.clone(), context.clone());
            self.throttle(cost.clone()).await;
        };
        self.set("last_rest_request_timestamp".into(), self.milliseconds());
        let mut request: Value = Qtrade::sign(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone());
    ...
    async fn request(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        config = config.or_default(Value::new_object());
        context = context.or_default(Value::new_object());
        return Qtrade::fetch2(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone(), config.clone(), context.clone()).await;
    }
    ...
    async fn fetch_ohlcvc(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        Qtrade::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut trades: Value = Qtrade::fetch_trades(self, symbol.clone(), since.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_trading_view_ohlcv(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        timeframe = timeframe.or_default(Value::from("1m"));
        let mut result: Value = Qtrade::convert_trading_view_to_ohlcv(self, ohlcvs.clone(), Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined);
        return Qtrade::parse_ohlcvs(self, result.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    async fn edit_limit_buy_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_sell_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_order(&mut self, mut id: Value, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::edit_order(self, id.clone(), symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn edit_order(&mut self, mut id: Value, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Qtrade::cancel_order(self, id.clone(), symbol.clone(), Value::Undefined).await;
        return Qtrade::create_order(self, symbol.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    fn parse_bid_ask(&self, mut bidask: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        price_key = price_key.or_default(Value::from(0));
        amount_key = amount_key.or_default(Value::from(1));
        let mut price: Value = Qtrade::safe_number(self, bidask.clone(), price_key.clone(), Value::Undefined);
        let mut amount: Value = Qtrade::safe_number(self, bidask.clone(), amount_key.clone(), Value::Undefined);
    ...
    fn safe_currency(&self, mut currency_id: Value, mut currency: Value) -> Value {
        ...
        if currency_id.clone().is_nonnullish() {
            code = Qtrade::common_currency_code(self, currency_id.to_upper_case());
        };
        return Value::Json(normalize(&Value::Json(json!({
            "id": currency_id,
            "code": code
        ...
        if market_id.clone().is_nonnullish() {
            if self.get("markets_by_id".into()).is_nonnullish() && self.get("markets_by_id".into()).contains_key(market_id.clone()) {
                ...
                if parts_length.clone() == Value::from(2) {
                    ...
                    result.set("base".into(), Qtrade::safe_currency_code(self, result.get(Value::from("baseId")), Value::Undefined));
                    result.set("quote".into(), Qtrade::safe_currency_code(self, result.get(Value::from("quoteId")), Value::Undefined));
    ...
    async fn fetch_partial_balance(&mut self, mut part: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut balance: Value = Qtrade::fetch_balance(self, params.clone()).await;
        return balance.get(part.clone());
    }

    async fn fetch_free_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::fetch_partial_balance(self, Value::from("free"), params.clone()).await;
    }

    async fn fetch_used_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::fetch_partial_balance(self, Value::from("used"), params.clone()).await;
    }

    async fn fetch_total_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::fetch_partial_balance(self, Value::from("total"), params.clone()).await;
    }
    ...
    async fn fetch_funding_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Qtrade::fetch_transaction_fee(self, code.clone(), params.clone()).await;
    }
    ...
    async fn fetch_funding_fees(&mut self, mut codes: Value, mut params: Value) -> Value {
        ...
        return Qtrade::fetch_transaction_fees(self, codes.clone(), params.clone()).await;
    }
    ...
    async fn fetch_transaction_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Qtrade::fetch_transaction_fees(self, Value::Json(serde_json::Value::Array(vec![code.clone().into()])), params.clone()).await;
    }
    ...
    fn throw_broadly_matched_exception(&mut self, mut broad: Value, mut string: Value, mut message: Value) -> () {
        let mut broad_key: Value = Qtrade::find_broadly_matched_key(self, broad.clone(), string.clone());
    ...
    async fn fetch_order_status(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut order: Value = Qtrade::fetch_order(self, id.clone(), symbol.clone(), params.clone()).await;
        return order.get(Value::from("status"));
    }

    async fn fetch_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::fetch_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }

    async fn cancel_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::cancel_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }
    ...
    async fn create_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn create_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("limit"), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("limit"), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_market_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("market"), Value::from("buy"), amount.clone(), Value::Undefined, params.clone()).await;
    }

    async fn create_market_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Qtrade::create_order(self, symbol.clone(), Value::from("market"), Value::from("sell"), amount.clone(), Value::Undefined, params.clone()).await;
    }
    ...
    async fn create_post_only_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        return Qtrade::create_order(self, symbol.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Qtrade::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Qtrade::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), Value::Undefined, query.clone()).await;
    }

    fn safe_currency_code(&self, mut currency_id: Value, mut currency: Value) -> Value {
        currency = Qtrade::safe_currency(self, currency_id.clone(), currency.clone());
    ...
    fn parse_tickers(&self, mut tickers: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(tickers.clone()).is_truthy() {
            let mut i: usize = 0;
            while i < tickers.len() {
                let mut ticker: Value = extend_2(Qtrade::parse_ticker(self, tickers.get(i.into()), Value::Undefined), params.clone());
            ...
            while i < market_ids.len() {
                ...
                let mut market: Value = Qtrade::safe_market(self, market_id.clone(), Value::Undefined, Value::Undefined);
                let mut ticker: Value = extend_2(Qtrade::parse_ticker(self, tickers.get(market_id.clone()), market.clone()), params.clone());
                results.push(ticker.clone());
                i += 1;
            };
        };
        symbols = Qtrade::market_symbols(self, symbols.clone());
        return Qtrade::filter_by_array(self, results.clone(), Value::from("symbol"), symbols.clone(), Value::Undefined);
    }
    ...
    fn parse_deposit_addresses(&self, mut addresses: Value, mut codes: Value, mut indexed: Value, mut params: Value) -> Value {
        ...
        while i < addresses.len() {
            let mut address: Value = extend_2(Qtrade::parse_deposit_address(self, addresses.get(i.into()), Value::Undefined), params.clone());
            result.push(address.clone());
            i += 1;
        };
        if codes.clone().is_nonnullish() {
            result = Qtrade::filter_by_array(self, result.clone(), Value::from("currency"), codes.clone(), false.into());
        };
    ...
    fn parse_funding_rate_histories(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        return Qtrade::filter_by_symbol_since_limit(self, sorted.clone(), symbol.clone(), since.clone(), limit.clone(), Value::Undefined);
    }

    fn safe_symbol(&self, mut market_id: Value, mut market: Value, mut delimiter: Value) -> Value {
        market = Qtrade::safe_market(self, market_id.clone(), market.clone(), delimiter.clone());
    ...
    fn parse_funding_rates(&self, mut response: Value, mut market: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < response.len() {
            let mut parsed: Value = Qtrade::parse_funding_rate(self, response.get(i.into()), market.clone());
    ...
    fn parse_open_interests(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        while i < response.len() {
            let mut entry: Value = response.get(i.into());
            let mut interest: Value = Qtrade::parse_open_interest(self, entry.clone(), market.clone());
    ...
    async fn fetch_mark_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if self.get("has".into()).get(Value::from("fetchMarkOHLCV")).is_truthy() {
            ...
            return Qtrade::fetch_ohlcv(self, symbol.clone(), timeframe.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
        } else {
    ...
    fn parse_account(&self, mut account: Value) -> Value {
        ...
        if accounts_by_type.contains_key(account.clone()) {
            ...
            let mut market: Value = Qtrade::market(self, account.clone());
            return market.get(Value::from("id"));
        } else {
    ...
    async fn load_markets_helper(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if !reload.is_truthy() && self.get("markets".into()).is_truthy() {
            if !self.get("markets_by_id".into()).is_truthy() {
                return Qtrade::set_markets(self, self.get("markets".into()), Value::Undefined);
            };
            return self.get("markets".into());
        };
        ...
        if self.get("has".into()).get(Value::from("fetchCurrencies")) == true.into() {
            currencies = Qtrade::fetch_currencies(self, Value::Undefined).await;
        };
        let mut markets: Value = Qtrade::fetch_markets(self, params.clone()).await;
        return Qtrade::set_markets(self, markets.clone(), currencies.clone());
    }
    ...
    async fn load_markets(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if reload.is_truthy() && !self.get("reloading_markets".into()).is_truthy() || !self.get("markets_loading".into()).is_truthy() {
            ...
            let mut markets_loading: Value = Qtrade::load_markets_helper(self, reload.clone(), params.clone()).await;
    ...
    async fn dispatch(&mut self, method: Value, params: Value, context: Value) -> Value {
        match method {
            Value::Json(serde_json::Value::String(ref m)) => {
                match m.as_ref() {
                    "publicGetTickerMarketString" => Qtrade::request(self, "ticker/{market_string}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetTickers" => Qtrade::request(self, "tickers".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetCurrencyCode" => Qtrade::request(self, "currency/{code}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetCurrencies" => Qtrade::request(self, "currencies".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetCommon" => Qtrade::request(self, "common".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetMarketMarketString" => Qtrade::request(self, "market/{market_string}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetMarkets" => Qtrade::request(self, "markets".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetMarketMarketStringTrades" => Qtrade::request(self, "market/{market_string}/trades".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetOrderbookMarketString" => Qtrade::request(self, "orderbook/{market_string}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetMarketMarketStringOhlcvInterval" => Qtrade::request(self, "market/{market_string}/ohlcv/{interval}".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetMe" => Qtrade::request(self, "me".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetBalances" => Qtrade::request(self, "balances".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetBalancesAll" => Qtrade::request(self, "balances_all".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetMarketMarketString" => Qtrade::request(self, "market/{market_string}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetOrders" => Qtrade::request(self, "orders".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetOrderOrderId" => Qtrade::request(self, "order/{order_id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetTrades" => Qtrade::request(self, "trades".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetWithdrawWithdrawId" => Qtrade::request(self, "withdraw/{withdraw_id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetWithdraws" => Qtrade::request(self, "withdraws".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetDepositDepositId" => Qtrade::request(self, "deposit/{deposit_id}".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetDeposits" => Qtrade::request(self, "deposits".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privateGetTransfers" => Qtrade::request(self, "transfers".into(), "private".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostCancelOrder" => Qtrade::request(self, "cancel_order".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostWithdraw" => Qtrade::request(self, "withdraw".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostDepositAddressCurrency" => Qtrade::request(self, "deposit_address/{currency}".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostSellLimit" => Qtrade::request(self, "sell_limit".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostBuyLimit" => Qtrade::request(self, "buy_limit".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    _ => unimplemented!(),
                }
            },
            _ => unimplemented!()
...
pub struct QtradeImpl(Value);
impl Exchange for QtradeImpl {}
impl Qtrade for QtradeImpl {}
impl ValueTrait for QtradeImpl {
...
impl QtradeImpl {
    pub fn new(params: Value) -> Self {
        let mut rv = QtradeImpl(match params {
            Value::Json(_) => params,
            _ => Value::new_object()
        });
        ExchangeImpl::init(&mut rv.0);

        let config_entries = Qtrade::describe(&rv);
