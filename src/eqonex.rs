...
pub trait Eqonex : Exchange {
    ...
    async fn fetch_markets(&mut self, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetInstrumentPairs".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //    {
        //        "instrumentPairs": [
        //            {
        ...
        //    }
        //
        let mut instrument_pairs: Value = self.safe_value(response.clone(), Value::from("instrumentPairs"), Value::new_array());
        let mut markets: Value = Value::new_array();
        let mut i: usize = 0;
        while i < instrument_pairs.len() {
            let mut market: Value = Eqonex::parse_market(self, instrument_pairs.get(i.into()));
    ...
    fn parse_market(&self, mut market: Value) -> Value {
        ...
        //    }
        //
        let mut asset_type: Value = self.safe_string(market.clone(), Value::from("assetType"), Value::Undefined);
        let mut spot: Value = (asset_type.clone() == Value::from("PAIR")).into();
        let mut swap: Value = (asset_type.clone() == Value::from("PERPETUAL_SWAP")).into();
        let mut future: Value = (asset_type.clone() == Value::from("DATED_FUTURE")).into();
        ...
        let mut base_id: Value = self.safe_string(market.clone(), Value::from("currency"), Value::Undefined);
        let mut quote_id: Value = self.safe_string(market.clone(), Value::from("contAmtCurr"), Value::Undefined);
        let mut settle_id: Value = if contract.is_truthy() { self.safe_string(market.clone(), Value::from("settlCurrency"), Value::Undefined) } else { Value::Undefined };
        let mut base: Value = Eqonex::safe_currency_code(self, base_id.clone(), Value::Undefined);
        let mut quote: Value = Eqonex::safe_currency_code(self, quote_id.clone(), Value::Undefined);
        let mut settle: Value = Eqonex::safe_currency_code(self, settle_id.clone(), Value::Undefined);
        let mut symbol: Value = base.clone() + Value::from("/") + quote.clone();
        let mut uppercase_id: Value = self.safe_string(market.clone(), Value::from("symbol"), Value::Undefined);
        let mut r#type: Value = Value::from("spot");
        let mut linear: Value = Value::Undefined;
        let mut inverse: Value = Value::Undefined;
        let mut expiry: Value = Eqonex::safe_number(self, market.clone(), Value::from("contractExpireTime"), Value::Undefined);
        if contract.is_truthy() {
            symbol = symbol.clone() + Value::from(":") + settle.clone();
            linear = (quote.clone() == settle.clone()).into();
            ...
            if swap.is_truthy() {
                ...
                symbol = uppercase_id.clone();
                r#type = asset_type.clone();
            };
        };
        let mut status: Value = self.safe_integer(market.clone(), Value::from("securityStatus"), Value::Undefined);
        return Value::Json(normalize(&Value::Json(json!({
            ...
            "future": future,
            "option": false,
            "active": status.clone() == Value::from(1),
            "contract": contract,
            "linear": linear,
            "inverse": inverse,
            "contractSize": Eqonex::safe_number(self, market.clone(), Value::from("contractMultiplier"), Value::Undefined),
            "expiry": expiry,
            "expiryDatetime": self.iso8601(expiry.clone()),
            "strike": Value::Undefined,
            "optionType": Value::Undefined,
            "precision": Value::Json(normalize(&Value::Json(json!({
                "amount": self.parse_number(Eqonex::parse_precision(self, self.safe_string(market.clone(), Value::from("quantity_scale"), Value::Undefined)), Value::Undefined),
                "price": self.parse_number(Eqonex::parse_precision(self, self.safe_string(market.clone(), Value::from("price_scale"), Value::Undefined)), Value::Undefined)
            }))).unwrap()),
            "limits": Value::Json(normalize(&Value::Json(json!({
                ...
                "amount": Value::Json(normalize(&Value::Json(json!({
                    "min": Eqonex::safe_number(self, market.clone(), Value::from("minTradeVol"), Value::Undefined),
                    "max": Value::Undefined
                }))).unwrap()),
    ...
    /// # Arguments
    ///
    /// * `params` {object} - extra parameters specific to the eqonex api endpoint
    async fn fetch_currencies(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetInstruments".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         "instruments": [
        //             [
        ...
        while i < instruments.len() {
            let mut currency: Value = Eqonex::parse_currency(self, instruments.get(i.into()));
            let mut code: Value = currency.get(Value::from("code"));
            currencies.set(code.clone(), currency.clone());
            i += 1;
        };
    ...
    fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        let mut uppercase_id: Value = self.safe_string(currency.clone(), Value::from(1), Value::Undefined);
        let mut code: Value = Eqonex::safe_currency_code(self, uppercase_id.clone(), Value::Undefined);
        let mut name: Value = self.safe_string(currency.clone(), Value::from(6), Value::Undefined);
        let mut status: Value = self.safe_integer(currency.clone(), Value::from(4), Value::Undefined);
        let mut active: Value = (status.clone() == Value::from(1)).into();
        let mut fee: Value = Eqonex::safe_number(self, currency.clone(), Value::from(5), Value::Undefined);
        // withdraw_fee
        return Value::Json(normalize(&Value::Json(json!({
            "id": id,
            "info": currency,
            ...
            "code": code,
            "name": name,
            "precision": self.parse_number(Eqonex::parse_precision(self, self.safe_string(currency.clone(), Value::from(3), Value::Undefined)), Value::Undefined),
            "fee": fee,
            "active": active,
    ...
    async fn fetch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        params = params.or_default(Value::new_object());
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut market: Value = Eqonex::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "pairId": parse_int(market.get(Value::from("id"))),
            "timespan": self.get("timeframes".into()).get(timeframe.clone())
        }))).unwrap());
        if limit.clone().is_nonnullish() {
            request.set("limit".into(), limit.clone());
        };
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetChart".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "pairId":57,
        //         "t":1,
        ...
        //     }
        //
        let mut chart: Value = self.safe_value(response.clone(), Value::from("chart"), Value::new_array());
        return Eqonex::parse_ohlcvs(self, chart.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    fn parse_ohlcv(&self, mut ohlcv: Value, mut market: Value) -> Value {
        ...
        let mut open: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(ohlcv.clone(), Value::from(1), Value::Undefined), market.get(Value::from("precision")).get(Value::from("price"))), Value::Undefined);
        let mut high: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(ohlcv.clone(), Value::from(2), Value::Undefined), market.get(Value::from("precision")).get(Value::from("price"))), Value::Undefined);
        let mut low: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(ohlcv.clone(), Value::from(3), Value::Undefined), market.get(Value::from("precision")).get(Value::from("price"))), Value::Undefined);
        let mut close: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(ohlcv.clone(), Value::from(4), Value::Undefined), market.get(Value::from("precision")).get(Value::from("price"))), Value::Undefined);
        let mut volume: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(ohlcv.clone(), Value::from(5), Value::Undefined), market.get(Value::from("precision")).get(Value::from("amount"))), Value::Undefined);
    ...
    fn parse_bid_ask(&self, mut bidask: Value, mut price_key: Value, mut amount_key: Value, mut market: Value) -> Value {
        ...
        let mut price_string: Value = self.safe_string(bidask.clone(), price_key.clone(), Value::Undefined);
        let mut amount_string: Value = self.safe_string(bidask.clone(), amount_key.clone(), Value::Undefined);
        return Value::Json(serde_json::Value::Array(vec![self.parse_number(Eqonex::convert_from_scale(self, price_string.clone(), market.get(Value::from("precision")).get(Value::from("price"))), Value::Undefined).into(), self.parse_number(Eqonex::convert_from_scale(self, amount_string.clone(), market.get(Value::from("precision")).get(Value::from("amount"))), Value::Undefined).into()]));
    }
    ...
    fn parse_order_book(&self, mut orderbook: Value, mut symbol: Value, mut timestamp: Value, mut bids_key: Value, mut asks_key: Value, mut price_key: Value, mut amount_key: Value, mut market: Value) -> Value {
        ...
        while i < sides.len() {
            ...
            let mut k: usize = 0;
            while k < bidasks.len() {
                orders.push(Eqonex::parse_bid_ask(self, bidasks.get(k.into()), price_key.clone(), amount_key.clone(), market.clone()));
                k += 1;
            };
    ...
    async fn fetch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetOrderBook".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "bids":[
        ...
        //     }
        //
        return Eqonex::parse_order_book(self, response.clone(), market.get(Value::from("symbol")), Value::Undefined, Value::from("bids"), Value::from("asks"), Value::from(0), Value::from(1), market.clone());
    }
    ...
    async fn fetch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetTradeHistory".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "trades":[
        ...
        return Eqonex::parse_trades(self, trades.clone(), market.clone(), since.clone(), limit.clone(), params.clone());
    }
    ...
    fn parse_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        if Array::is_array(trade.clone()).is_truthy() {
            id = self.safe_string(trade.clone(), Value::from(3), Value::Undefined);
            price_string = Eqonex::convert_from_scale(self, self.safe_string(trade.clone(), Value::from(0), Value::Undefined), market.get(Value::from("precision")).get(Value::from("price")));
            amount_string = Eqonex::convert_from_scale(self, self.safe_string(trade.clone(), Value::from(1), Value::Undefined), market.get(Value::from("precision")).get(Value::from("amount")));
            timestamp = Eqonex::to_milliseconds(self, self.safe_string(trade.clone(), Value::from(2), Value::Undefined));
            let mut taker_side: Value = self.safe_integer(trade.clone(), Value::from(4), Value::Undefined);
            if taker_side.clone() == Value::from(1) {
                side = Value::from("buy");
            } else if taker_side.clone() == Value::from(2) {
                side = Value::from("sell");
            };
        } else {
            id = self.safe_string(trade.clone(), Value::from("execId"), Value::Undefined);
            timestamp = self.safe_integer(trade.clone(), Value::from("time"), Value::Undefined);
            market_id = self.safe_string(trade.clone(), Value::from("symbol"), Value::Undefined);
            ...
            r#type = Eqonex::parse_order_type(self, self.safe_string(trade.clone(), Value::from("ordType"), Value::Undefined));
            price_string = self.safe_string(trade.clone(), Value::from("lastPx"), Value::Undefined);
            amount_string = self.safe_string(trade.clone(), Value::from("qty"), Value::Undefined);
            ...
            if fee_cost_string.clone().is_nonnullish() {
                ...
                let mut fee_currency_code: Value = Eqonex::safe_currency_code(self, fee_currency_id.clone(), Value::Undefined);
        ...
        market = Eqonex::safe_market(self, market_id.clone(), market.clone(), Value::Undefined);
        return Eqonex::safe_trade(self, Value::Json(normalize(&Value::Json(json!({
            "info": trade,
            "id": id,
            "timestamp": timestamp,
            "datetime": self.iso8601(timestamp.clone()),
    ...
    fn parse_balance(&self, mut response: Value) -> Value {
        let mut positions: Value = self.safe_value(response.clone(), Value::from("positions"), Value::new_array());
        let mut result: Value = Value::Json(normalize(&Value::Json(json!({
            "info": response
        }))).unwrap());
        let mut i: usize = 0;
        while i < positions.len() {
            ...
            let mut asset_type: Value = self.safe_string(position.clone(), Value::from("assetType"), Value::Undefined);
            if asset_type.clone() == Value::from("ASSET") {
                let mut currency_id: Value = self.safe_string(position.clone(), Value::from("symbol"), Value::Undefined);
                let mut code: Value = Eqonex::safe_currency_code(self, currency_id.clone(), Value::Undefined);
                let mut quantity_string: Value = self.safe_string(position.clone(), Value::from("quantity"), Value::Undefined);
                let mut available_quantity_string: Value = self.safe_string(position.clone(), Value::from("availableQuantity"), Value::Undefined);
                let mut scale: Value = self.safe_integer(position.clone(), Value::from("quantity_scale"), Value::Undefined);
                let mut account: Value = Eqonex::account(self);
                account.set("free".into(), Eqonex::convert_from_scale(self, available_quantity_string.clone(), scale.clone()));
                account.set("total".into(), Eqonex::convert_from_scale(self, quantity_string.clone(), scale.clone()));
                result.set(code.clone(), account.clone());
            };
            i += 1;
        };
        return Eqonex::safe_balance(self, result.clone());
    }
    ...
    async fn fetch_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetPositions".into(), params.clone(), Value::Undefined).await;
        //     {
        //         "positions":[
        //             {
        //                 "instrumentId":1,
        ...
        //         ]
        //     }
        return Eqonex::parse_balance(self, response.clone());
    }
    ...
    async fn create_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        let mut quantity_scale: Value = Eqonex::get_scale(self, amount.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "instrumentId": parse_int(market.get(Value::from("id"))),
            "symbol": market.get(Value::from("uppercaseId")),
            "side": order_side,
            "quantity": Eqonex::convert_to_scale(self, self.number_to_string(amount.clone()), quantity_scale.clone()),
        ...
        if r#type.clone() == Value::from("market") {
            request.set("ordType".into(), Value::from(1));
        } else if r#type.clone() == Value::from("limit") {
            request.set("ordType".into(), Value::from(2));
            let mut price_scale: Value = Eqonex::get_scale(self, price.clone());
            request.set("price".into(), Eqonex::convert_to_scale(self, self.number_to_string(price.clone()), price_scale.clone()));
            request.set("priceScale".into(), price_scale.clone());
        } else {
            let mut stop_price: Value = Eqonex::safe_number_2(self, params.clone(), Value::from("stopPrice"), Value::from("stopPx"), Value::Undefined);
            params = self.omit(params.clone(), Value::Json(serde_json::Value::Array(vec![Value::from("stopPrice").into(), Value::from("stopPx").into()])));
            if stop_price.clone().is_nullish() {
                if r#type.clone() == Value::from("stop") {
                    if price.clone().is_nullish() {
                        panic!(r###"ArgumentsRequired::new(self.get("id".into()) + Value::from(" createOrder() requires a price argument or a stopPrice parameter or a stopPx parameter for ") + r#type.clone() + Value::from(" orders"))"###);
                    };
                    request.set("ordType".into(), Value::from(3));
                    request.set("stopPx".into(), Eqonex::convert_to_scale(self, self.number_to_string(price.clone()), Eqonex::get_scale(self, price.clone())));
                } else if r#type.clone() == Value::from("stop limit") {
                    panic!(r###"ArgumentsRequired::new(self.get("id".into()) + Value::from(" createOrder() requires a stopPrice parameter or a stopPx parameter for ") + r#type.clone() + Value::from(" orders"))"###);
                };
            } else {
                if r#type.clone() == Value::from("stop") {
                    request.set("ordType".into(), Value::from(3));
                    request.set("stopPx".into(), Eqonex::convert_to_scale(self, self.number_to_string(stop_price.clone()), Eqonex::get_scale(self, stop_price.clone())));
                } else if r#type.clone() == Value::from("stop limit") {
                    request.set("ordType".into(), Value::from(4));
                    let mut price_scale: Value = Eqonex::get_scale(self, price.clone());
                    let mut stop_price_scale: Value = Eqonex::get_scale(self, stop_price.clone());
                    request.set("price_scale".into(), price_scale.clone());
                    request.set("stopPx_scale".into(), stop_price_scale.clone());
                    request.set("stopPx".into(), Eqonex::convert_to_scale(self, self.number_to_string(stop_price.clone()), stop_price_scale.clone()));
                    request.set("price".into(), Eqonex::convert_to_scale(self, self.number_to_string(price.clone()), price_scale.clone()));
                };
            };
        };
        let mut response: Value = Eqonex::dispatch(self, "privatePostOrder".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "status":"sent",
        //         "id":385617863,
        ...
        //     }
        //
        return Eqonex::parse_order(self, response.clone(), market.clone());
    }
    ...
    async fn cancel_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut market: Value = Eqonex::market(self, symbol.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "origOrderId": parse_int(id.clone()),
            "instrumentId": parse_int(market.get(Value::from("id")))
        }))).unwrap());
        let mut response: Value = Eqonex::dispatch(self, "privatePostCancelOrder".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "status":"sent",
        //         "id":0,
    ...
    async fn edit_order(&mut self, mut id: Value, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        if r#type.clone() == Value::from("market") {
            ...
            request.set("ordType".into(), Value::from(2));
            request.set("price".into(), Eqonex::convert_to_scale(self, self.number_to_string(price.clone()), Eqonex::get_scale(self, price.clone())));
        } else {
            let mut stop_price: Value = Eqonex::safe_number_2(self, params.clone(), Value::from("stopPrice"), Value::from("stopPx"), Value::Undefined);
            params = self.omit(params.clone(), Value::Json(serde_json::Value::Array(vec![Value::from("stopPrice").into(), Value::from("stopPx").into()])));
            if stop_price.clone().is_nullish() {
                if r#type.clone() == Value::from("stop") {
                    if price.clone().is_nullish() {
                        panic!(r###"ArgumentsRequired::new(self.get("id".into()) + Value::from(" editOrder() requires a price argument or a stopPrice parameter or a stopPx parameter for ") + r#type.clone() + Value::from(" orders"))"###);
                    };
                    request.set("ordType".into(), Value::from(3));
                    request.set("stopPx".into(), Eqonex::convert_to_scale(self, self.number_to_string(price.clone()), Eqonex::get_scale(self, price.clone())));
                } else if r#type.clone() == Value::from("stop limit") {
                    panic!(r###"ArgumentsRequired::new(self.get("id".into()) + Value::from(" editOrder() requires a stopPrice parameter or a stopPx parameter for ") + r#type.clone() + Value::from(" orders"))"###);
                };
            } else {
                if r#type.clone() == Value::from("stop") {
                    request.set("ordType".into(), Value::from(3));
    ...
    /// # Arguments
    ///
    /// * `symbol` {string|undefined} - not used by eqonex fetchOrder
    /// * `params` {object} - extra parameters specific to the eqonex api endpoint
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetOrderStatus".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "orderId":388953019,
        //         "clOrdId":"1613106766970339107",
        ...
        //     }
        //
        return Eqonex::parse_order(self, response.clone(), Value::Undefined);
    }
    ...
    async fn fetch_closed_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        // '0' = New, '1' = Partially filled, '2' = Filled, '4' = Cancelled, '8' = Rejected, 'C' = Expired
        return Eqonex::fetch_orders(self, symbol.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
    }
    ...
    async fn fetch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if symbol.clone().is_nonnullish() {
            market = Eqonex::market(self, symbol.clone());
            request.set("instrumentId".into(), parse_int(market.get(Value::from("id"))));
        };
        if limit.clone().is_nonnullish() {
            request.set("limit".into(), limit.clone());
        };
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetOrders".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "isInitialSnap":false,
        //         "orders":[
        ...
        return Eqonex::parse_orders(self, orders.clone(), market.clone(), since.clone(), limit.clone(), params.clone());
    }
    ...
    async fn fetch_my_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "privatePostUserTrades".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "trades":[
        //             {
    ...
    async fn fetch_deposit_address(&mut self, mut code: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut currency: Value = Eqonex::currency(self, code.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "instrumentId": parse_int(currency.get(Value::from("id")))
        }))).unwrap());
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetDepositAddresses".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "addresses":[
        ...
        //     }
        //
        let mut addresses: Value = self.safe_value(response.clone(), Value::from("addresses"), Value::new_array());
        let mut address: Value = self.safe_value(addresses.clone(), Value::from(0), Value::Undefined);
        return Eqonex::parse_deposit_address(self, address.clone(), currency.clone());
    }
    ...
    fn parse_deposit_address(&self, mut deposit_address: Value, mut currency: Value) -> Value {
        ...
        let mut code: Value = Eqonex::safe_currency_code(self, currency_id.clone(), currency.clone());
    ...
    async fn fetch_deposits(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if code.clone().is_nonnullish() {
            currency = Eqonex::currency(self, code.clone());
            request.set("instrumentId".into(), parse_int(currency.get(Value::from("id"))));
        };
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetDepositHistory".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "deposits":[
        //             {
        ...
        return Eqonex::parse_transactions(self, deposits.clone(), currency.clone(), since.clone(), limit.clone(), Value::Json(normalize(&Value::Json(json!({
            "type": "deposit"
        }))).unwrap()));
    }
    ...
    async fn fetch_withdrawals(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut response: Value = Eqonex::dispatch(self, "privatePostGetWithdrawRequests".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "addresses":[
        //             {
        ...
        //     }
        //
        let mut withdrawals: Value = self.safe_value(response.clone(), Value::from("addresses"), Value::new_array());
        return Eqonex::parse_transactions(self, withdrawals.clone(), currency.clone(), since.clone(), limit.clone(), Value::Json(normalize(&Value::Json(json!({
            "type": "withdrawal"
        }))).unwrap()));
    }
    ...
    fn parse_transaction(&self, mut transaction: Value, mut currency: Value) -> Value {
        ...
        //     }
        //
        let mut id: Value = self.safe_string(transaction.clone(), Value::from("id"), Value::from("transactionId"));
        let mut txid: Value = self.safe_string(transaction.clone(), Value::from("transactionUuid"), Value::Undefined);
        ...
        if address.clone() == Value::from("null") {
            address = Value::Undefined;
        };
        let mut r#type: Value = self.safe_string(transaction.clone(), Value::from("type"), Value::Undefined);
        let mut amount: Value = Eqonex::safe_number(self, transaction.clone(), Value::from("balance_change"), Value::Undefined);
        if amount.clone().is_nullish() {
            amount = self.safe_string(transaction.clone(), Value::from("quantity"), Value::Undefined);
            let mut amount_scale: Value = self.safe_integer(transaction.clone(), Value::from("quantity_scale"), Value::Undefined);
            amount = self.parse_number(Eqonex::convert_from_scale(self, amount.clone(), amount_scale.clone()), Value::Undefined);
        };
        let mut currency_id: Value = self.safe_string(transaction.clone(), Value::from("symbol"), Value::Undefined);
        let mut code: Value = Eqonex::safe_currency_code(self, currency_id.clone(), currency.clone());
        let mut status: Value = Eqonex::parse_transaction_status(self, self.safe_string(transaction.clone(), Value::from("status"), Value::Undefined));
        return Value::Json(normalize(&Value::Json(json!({
            "info": transaction,
            "id": id,
            "txid": txid,
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        (tag, params) = shift_2(Eqonex::handle_withdraw_tag_and_params(self, tag.clone(), params.clone()));
        self.check_address(address.clone());
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut currency: Value = Eqonex::currency(self, code.clone());
        let mut scale: Value = Eqonex::get_scale(self, amount.clone());
        let mut quantity: Value = Eqonex::convert_to_scale(self, amount.clone(), scale.clone());
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "instrumentId": parse_int(currency.get(Value::from("id"))),
            "symbol": currency.get(Value::from("uppercaseId")),
            "quantity": quantity,
            "quantity_scale": scale,
            "address": address
        }))).unwrap());
        let mut response: Value = Eqonex::dispatch(self, "privatePostSendWithdrawRequest".into(), extend_2(request.clone(), params.clone()), Value::Undefined).await;
        //
        //     {
        //         "instrumentId": 1,
        //         "userId": 23750,
        ...
        //     }
        //
        return Eqonex::parse_transaction(self, response.clone(), currency.clone());
    }
    ...
    async fn fetch_trading_fees(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetExchangeInfo".into(), params.clone(), Value::Undefined).await;
        //
        //     {
        //         tradingLimits: [],
        ...
        //     }
        //
        let mut spot_fees: Value = self.safe_value(response.clone(), Value::from("spotFees"), Value::new_array());
        let mut first_spot_fee: Value = self.safe_value(spot_fees.clone(), Value::from(0), Value::new_object());
        let mut spot_maker_fee: Value = Eqonex::safe_number(self, first_spot_fee.clone(), Value::from("maker"), Value::Undefined);
        let mut spot_taker_fee: Value = Eqonex::safe_number(self, first_spot_fee.clone(), Value::from("taker"), Value::Undefined);
        let mut future_fees: Value = self.safe_value(response.clone(), Value::from("futuresFees"), Value::new_array());
        let mut first_future_fee: Value = self.safe_value(future_fees.clone(), Value::from(0), Value::new_object());
        let mut future_maker_fee: Value = Eqonex::safe_number(self, first_future_fee.clone(), Value::from("maker"), Value::Undefined);
        let mut future_taker_fee: Value = Eqonex::safe_number(self, first_future_fee.clone(), Value::from("taker"), Value::Undefined);
        let mut spot_taker_tiers: Value = Value::new_array();
        let mut spot_maker_tiers: Value = Value::new_array();
        let mut result: Value = Value::new_object();
        let mut i: usize = 0;
        while i < spot_fees.len() {
            let mut spot_fee: Value = spot_fees.get(i.into());
            let mut volume: Value = Eqonex::safe_number(self, spot_fee.clone(), Value::from("volume"), Value::Undefined);
            spot_taker_tiers.push(Value::Json(serde_json::Value::Array(vec![volume.clone().into(), Eqonex::safe_number(self, spot_fee.clone(), Value::from("taker"), Value::Undefined).into()])));
            spot_maker_tiers.push(Value::Json(serde_json::Value::Array(vec![volume.clone().into(), Eqonex::safe_number(self, spot_fee.clone(), Value::from("maker"), Value::Undefined).into()])));
            i += 1;
        };
        let mut spot_tiers: Value = Value::Json(normalize(&Value::Json(json!({
            "taker": spot_taker_tiers,
            "maker": spot_maker_tiers
        }))).unwrap());
        let mut future_taker_tiers: Value = Value::new_array();
        let mut future_maker_tiers: Value = Value::new_array();
        let mut i: usize = 0;
        while i < future_fees.len() {
            let mut future_fee: Value = future_fees.get(i.into());
            future_taker_tiers.push(Value::Json(serde_json::Value::Array(vec![Value::Undefined.into(), Eqonex::safe_number(self, future_fee.clone(), Value::from("taker"), Value::Undefined).into()])));
            future_maker_tiers.push(Value::Json(serde_json::Value::Array(vec![Value::Undefined.into(), Eqonex::safe_number(self, future_fee.clone(), Value::from("maker"), Value::Undefined).into()])));
            i += 1;
        };
        let mut future_tiers: Value = Value::Json(normalize(&Value::Json(json!({
            "taker": future_taker_tiers,
            "maker": future_maker_tiers
        }))).unwrap());
        let mut i: usize = 0;
        while i < self.get("symbols".into()).len() {
            ...
            if self.safe_value(market.clone(), Value::from("spot"), Value::Undefined).is_truthy() {
                fee.set("maker".into(), spot_maker_fee.clone());
                fee.set("taker".into(), spot_taker_fee.clone());
                fee.set("tiers".into(), spot_tiers.clone());
            } else if self.safe_value(market.clone(), Value::from("contract"), Value::Undefined).is_truthy() {
                fee.set("maker".into(), future_maker_fee.clone());
                fee.set("taker".into(), future_taker_fee.clone());
                fee.set("tiers".into(), future_tiers.clone());
            };
    ...
    async fn fetch_trading_limits(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        // getExchangeInfo
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetExchangeInfo".into(), params.clone(), Value::Undefined).await;
        let mut trading_limits: Value = self.safe_value(response.clone(), Value::from("tradingLimits"), Value::new_array());
    ...
    async fn fetch_funding_limits(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        // getExchangeInfo
        let mut response: Value = Eqonex::dispatch(self, "publicGetGetExchangeInfo".into(), params.clone(), Value::Undefined).await;
        let mut withdraw_limits: Value = self.safe_value(response.clone(), Value::from("withdrawLimits"), Value::new_array());
        // TO-DO parse response when available
        return Value::Json(normalize(&Value::Json(json!({
            "info": withdraw_limits,
            "withdraw": Value::Undefined
        }))).unwrap());
    }
    ...
    fn parse_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        //     }
        //
        let mut id: Value = self.safe_string_2(order.clone(), Value::from("orderId"), Value::from("id"), Value::Undefined);
        id = self.safe_string(order.clone(), Value::from("origOrderId"), id.clone());
        let mut client_order_id: Value = self.safe_string(order.clone(), Value::from("clOrdId"), Value::Undefined);
        let mut r#type: Value = Eqonex::parse_order_type(self, self.safe_string(order.clone(), Value::from("ordType"), Value::Undefined));
        let mut side: Value = Eqonex::parse_order_side(self, self.safe_string(order.clone(), Value::from("side"), Value::Undefined));
        let mut status: Value = Eqonex::parse_order_status(self, self.safe_string(order.clone(), Value::from("ordStatus"), Value::Undefined));
        let mut market_id: Value = self.safe_string(order.clone(), Value::from("instrumentId"), Value::Undefined);
        let mut symbol: Value = Eqonex::safe_symbol(self, market_id.clone(), market.clone(), Value::Undefined);
        let mut timestamp: Value = Eqonex::to_milliseconds(self, self.safe_string(order.clone(), Value::from("timeStamp"), Value::Undefined));
        ...
        let mut price_scale: Value = self.safe_integer(order.clone(), Value::from("price_scale"), Value::Undefined);
        price_string = Eqonex::convert_from_scale(self, price_string.clone(), price_scale.clone());
        let mut amount_string: Value = self.safe_string(order.clone(), Value::from("quantity"), Value::Undefined);
        let mut amount_scale: Value = self.safe_integer(order.clone(), Value::from("quantity_scale"), Value::Undefined);
        amount_string = Eqonex::convert_from_scale(self, amount_string.clone(), amount_scale.clone());
        let mut filled_string: Value = self.safe_string(order.clone(), Value::from("cumQty"), Value::Undefined);
        let mut filled_scale: Value = self.safe_integer(order.clone(), Value::from("cumQty_scale"), Value::Undefined);
        filled_string = Eqonex::convert_from_scale(self, filled_string.clone(), filled_scale.clone());
        let mut remaining_string: Value = self.safe_string(order.clone(), Value::from("leavesQty"), Value::Undefined);
        let mut remaining_scale: Value = self.safe_integer(order.clone(), Value::from("leavesQty_scale"), Value::Undefined);
        remaining_string = Eqonex::convert_from_scale(self, remaining_string.clone(), remaining_scale.clone());
        let mut fee: Value = Value::Undefined;
        let mut currency_id: Value = self.safe_integer(order.clone(), Value::from("feeInstrumentId"), Value::Undefined);
        let mut fee_currency_code: Value = Eqonex::safe_currency_code(self, currency_id.clone(), Value::Undefined);
        let mut fee_cost_string: Value = Value::Undefined;
        let mut fee_cost: Value = self.safe_string(order.clone(), Value::from("feeTotal"), Value::Undefined);
        let mut fee_scale: Value = self.safe_integer(order.clone(), Value::from("fee_scale"), Value::Undefined);
        if fee_cost.clone().is_nonnullish() {
            fee_cost = Precise::string_neg(fee_cost.clone());
            fee_cost_string = Eqonex::convert_from_scale(self, fee_cost.clone(), fee_scale.clone());
        };
        if fee_cost.clone().is_nonnullish() {
            fee = Value::Json(normalize(&Value::Json(json!({
                "currency": fee_currency_code,
        ...
        let mut time_in_force: Value = Eqonex::parse_time_in_force(self, self.safe_string(order.clone(), Value::from("timeInForce"), Value::Undefined));
        if time_in_force.clone() == Value::from("0") {
            time_in_force = Value::Undefined;
        };
        let mut stop_price_scale: Value = self.safe_integer(order.clone(), Value::from("stopPx_scale"), Value::from(0));
        let mut stop_price: Value = self.parse_number(Eqonex::convert_from_scale(self, self.safe_string(order.clone(), Value::from("stopPx"), Value::Undefined), stop_price_scale.clone()), Value::Undefined);
        return Eqonex::safe_order(self, Value::Json(normalize(&Value::Json(json!({
            "info": order,
            "id": id,
    ...
    fn to_milliseconds(&mut self, mut date_string: Value) -> Value {
        if date_string.clone().is_nullish() {
            return date_string.clone();
        };
        // '20200328-10:31:01.575' -> '2020-03-28 12:42:48.000'
        let mut splits: Value = date_string.split(Value::from("-"));
        let mut part_one: Value = self.safe_string(splits.clone(), Value::from(0), Value::Undefined);
        let mut part_two: Value = self.safe_string(splits.clone(), Value::from(1), Value::Undefined);
        if part_one.clone().is_nullish() || part_two.clone().is_nullish() {
            return Value::Undefined;
        };
        if part_one.len() != 8 {
            return Value::Undefined;
        };
        let mut date: Value = part_one.slice(Value::from(0), Value::from(4)) + Value::from("-") + part_one.slice(Value::from(4), Value::from(6)) + Value::from("-") + part_one.slice(Value::from(6), Value::from(8));
        return self.parse8601(date.clone() + Value::from(" ") + part_two.clone());
    }

    fn convert_from_scale(&self, mut number: Value, mut scale_in_ticksize: Value) -> Value {
        let mut scale: Value = Eqonex::get_scale(self, scale_in_ticksize.clone());
        if number.clone().is_nullish() || scale.clone().is_nullish() {
            return Value::Undefined;
        };
        let mut precise: Value = Precise::new(number);
        precise.set("decimals".into(), precise.get(decimals.clone()) + scale.clone());
        precise.reduce();
        return precise.to_string();
    }

    fn get_scale(&mut self, mut num: Value) -> Value {
        let mut s: Value = self.number_to_string(num.clone());
        return self.precision_from_string(s.clone());
    }

    fn convert_to_scale(&self, mut number: Value, mut scale: Value) -> Value {
        if number.clone().is_nullish() || scale.clone().is_nullish() {
            return Value::Undefined;
        };
        let mut precise: Value = Precise::new(number);
        ...
        return parse_int(precise_string.clone());
    }
    ...
    fn handle_errors(&mut self, mut code: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut request_headers: Value, mut request_body: Value) -> Value {
        ...
        if error.clone().is_nonnullish() {
            ...
            Eqonex::throw_exactly_matched_exception(self, self.get("exceptions".into()), error.clone(), feedback.clone());
            Eqonex::throw_broadly_matched_exception(self, self.get("exceptions".into()), body.clone(), feedback.clone());
    ...
    fn sign(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value) -> Value {
        ...
        params = params.or_default(Value::new_object());
        let mut url: Value = self.implode_params(path.clone(), params.clone());
        ...
        if api.clone() == Value::from("public") {
            ...
            let mut format: Value = self.safe_value(params.clone(), Value::from("format"), Value::Undefined);
            let mut r#type: Value = self.safe_value(params.clone(), Value::from("type"), Value::Undefined);
            let mut extension: Value = Value::new_object();
            if format.clone().is_nonnullish() {
                extension.set("format".into(), format.clone());
            };
            if r#type.clone().is_nonnullish() {
                extension.set("type".into(), r#type.clone());
            };
            if Object::keys(extension.clone()).len() > 0 {
                url = url +  Value::from("?") + self.urlencode(extension.clone());
            };
            params = self.omit(params.clone(), Value::Json(serde_json::Value::Array(vec![Value::from("format").into(), Value::from("type").into()])));
            Eqonex::check_required_credentials(self, Value::Undefined);
            let mut nonce: Value = Eqonex::nonce(self);
            query = extend_2(query.clone(), Value::Json(normalize(&Value::Json(json!({
                "userId": self.get("uid".into()),
                "nonce": nonce
            }))).unwrap()));
            params.set("nonce".into(), Eqonex::nonce(self));
            body = self.json(query.clone(), Value::Undefined);
            let mut signature: Value = self.hmac(self.encode(body.clone()), self.encode(self.get("secret".into())), Value::from("sha384"), Value::Undefined);
            headers = Value::Json(normalize(&Value::Json(json!({
                "Content-Type": "application/json",
                "requestToken": self.get("apiKey".into()),
                "signature": signature
            }))).unwrap());
        };
        url = self.get("urls".into()).get(Value::from("api")).get(api.clone()) + Value::from("/") + url.clone();
        return Value::Json(normalize(&Value::Json(json!({
            "url": url,
            "method": method,
            "body": body,
    ...
    fn safe_ledger_entry(&self, mut entry: Value, mut currency: Value) -> Value {
        currency = Eqonex::safe_currency(self, Value::Undefined, currency.clone());
        ...
        if fee.clone().is_nonnullish() {
            fee.set("cost".into(), Eqonex::safe_number(self, fee.clone(), Value::from("cost"), Value::Undefined));
        };
    ...
    fn set_markets(&mut self, mut markets: Value, mut currencies: Value) -> Value {
        ...
        while i < market_values.len() {
            let mut market: Value = self.deep_extend_4(Eqonex::safe_market(self, Value::Undefined, Value::Undefined, Value::Undefined), Value::Json(normalize(&Value::Json(json!({
    ...
    fn safe_order(&mut self, mut order: Value, mut market: Value) -> Value {
        ...
        if parse_filled.is_truthy() || parse_cost.is_truthy() || should_parse_fees.is_truthy() {
            ...
            trades = Eqonex::parse_trades(self, raw_trades.clone(), market.clone(), Value::Undefined, Value::Undefined, Value::Json(normalize(&Value::Json(json!({
        ...
        if should_parse_fees.is_truthy() {
            let mut reduced_fees: Value = if self.get("reduce_fees".into()).is_truthy() { Eqonex::reduce_fees_by_currency(self, fees.clone()) } else { fees.clone() };
            let mut reduced_length: Value = reduced_fees.len().into();
            let mut i: usize = 0;
            while i < reduced_length.clone().into() {
                reduced_fees.get(i.into()).set("cost".into(), Eqonex::safe_number(self, reduced_fees.get(i.into()), Value::from("cost"), Value::Undefined));
                if reduced_fees.get(i.into()).contains_key(Value::from("rate")) {
                    reduced_fees.get(i.into()).set("rate".into(), Eqonex::safe_number(self, reduced_fees.get(i.into()), Value::from("rate"), Value::Undefined));
                };
                i += 1;
            };
            if !parse_fee.is_truthy() && reduced_length.clone() == Value::from(0) {
                ...
                if fee.contains_key(Value::from("rate")) {
                    fee.set("rate".into(), Eqonex::safe_number(self, fee.clone(), Value::from("rate"), Value::Undefined));
                };
        ...
        while i < trades.len() {
            ...
            entry.set("amount".into(), Eqonex::safe_number(self, entry.clone(), Value::from("amount"), Value::Undefined));
            entry.set("price".into(), Eqonex::safe_number(self, entry.clone(), Value::from("price"), Value::Undefined));
            entry.set("cost".into(), Eqonex::safe_number(self, entry.clone(), Value::from("cost"), Value::Undefined));
    ...
    fn parse_orders(&mut self, mut orders: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(orders.clone()).is_truthy() {
            let mut i: usize = 0;
            while i < orders.len() {
                let mut order: Value = extend_2(Eqonex::parse_order(self, orders.get(i.into()), market.clone()), params.clone());
            ...
            while i < ids.len() {
                ...
                let mut order: Value = extend_2(Eqonex::parse_order(self, extend_2(Value::Json(normalize(&Value::Json(json!({
                    "id": id
        ...
        return Eqonex::filter_by_symbol_since_limit(self, results.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn safe_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
        ...
        if should_parse_fees.is_truthy() {
            ...
            let mut trade_fee: Value = self.safe_value(trade.clone(), Value::from("fee"), Value::Undefined);
            if trade_fee.clone().is_nonnullish() {
                trade_fee.set("cost".into(), Eqonex::safe_number(self, trade_fee.clone(), Value::from("cost"), Value::Undefined));
                if trade_fee.contains_key(Value::from("rate")) {
                    trade_fee.set("rate".into(), Eqonex::safe_number(self, trade_fee.clone(), Value::from("rate"), Value::Undefined));
                };
    ...
    fn safe_ticker(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        return extend_2(ticker.clone(), Value::Json(normalize(&Value::Json(json!({
            "bid": Eqonex::safe_number(self, ticker.clone(), Value::from("bid"), Value::Undefined),
            "bidVolume": Eqonex::safe_number(self, ticker.clone(), Value::from("bidVolume"), Value::Undefined),
            "ask": Eqonex::safe_number(self, ticker.clone(), Value::from("ask"), Value::Undefined),
            "askVolume": Eqonex::safe_number(self, ticker.clone(), Value::from("askVolume"), Value::Undefined),
            "high": Eqonex::safe_number(self, ticker.clone(), Value::from("high"), Value::Undefined),
            "low": Eqonex::safe_number(self, ticker.clone(), Value::from("low"), Value::Undefined),
            ...
            "previousClose": Eqonex::safe_number(self, ticker.clone(), Value::from("previousClose"), Value::Undefined)
        }))).unwrap()));
    }
    ...
    fn market_ids(&mut self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Eqonex::market_id(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn market_symbols(&self, mut symbols: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < symbols.len() {
            result.push(Eqonex::symbol(self, symbols.get(i.into())));
            i += 1;
        };
        return result.clone();
    }
    ...
    fn parse_bids_asks(&self, mut bidasks: Value, mut price_key: Value, mut amount_key: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < bidasks.len() {
            result.push(Eqonex::parse_bid_ask(self, bidasks.get(i.into()), price_key.clone(), amount_key.clone(), Value::Undefined));
            i += 1;
        };
        return result.clone();
    }
    ...
    async fn fetch_l2_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut orderbook: Value = Eqonex::fetch_order_book(self, symbol.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_ohlcvs(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < ohlcvs.len() {
            results.push(Eqonex::parse_ohlcv(self, ohlcvs.get(i.into()), market.clone()));
            i += 1;
        };
    ...
    fn parse_leverage_tiers(&self, mut response: Value, mut symbols: Value, mut market_id_key: Value) -> Value {
        ...
        symbols = Eqonex::market_symbols(self, symbols.clone());
        ...
        while i < response.len() {
            ...
            let mut market: Value = Eqonex::safe_market(self, id.clone(), Value::Undefined, Value::Undefined);
    ...
    async fn load_trading_limits(&mut self, mut symbols: Value, mut reload: Value, mut params: Value) -> Value {
        ...
        if self.get("has".into()).get(Value::from("fetchTradingLimits")).is_truthy() {
            if reload.is_truthy() || !self.get("options".into()).contains_key(Value::from("limitsLoaded")) {
                let mut response: Value = Eqonex::fetch_trading_limits(self, symbols.clone(), Value::Undefined).await;
                let mut i: usize = 0;
                while i < symbols.len() {
    ...
    fn parse_positions(&self, mut positions: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        return Eqonex::filter_by_array(self, result.clone(), Value::from("symbol"), symbols.clone(), false.into());
    }
    ...
    fn parse_accounts(&self, mut accounts: Value, mut params: Value) -> Value {
        ...
        while i < accounts.len() {
            let mut account: Value = extend_2(Eqonex::parse_account(self, accounts.get(i.into())), params.clone());
    ...
    fn parse_trades(&mut self, mut trades: Value, mut market: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < trades.len() {
            let mut trade: Value = extend_2(Eqonex::parse_trade(self, trades.get(i.into()), market.clone()), params.clone());
        ...
        let mut symbol: Value = if market.clone().is_nonnullish() { market.get(Value::from("symbol")) } else { Value::Undefined };
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Eqonex::filter_by_symbol_since_limit(self, result.clone(), symbol.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    fn parse_transactions(&self, mut transactions: Value, mut currency: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        while i < transactions.len() {
            let mut transaction: Value = extend_2(Eqonex::parse_transaction(self, transactions.get(i.into()), currency.clone()), params.clone());
        ...
        let mut tail: Value = (since.clone().is_nullish()).into();
        return Eqonex::filter_by_currency_since_limit(self, result.clone(), code.clone(), since.clone(), limit.clone(), tail.clone());
    }
    ...
    async fn fetch2(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        if self.get("enable_rate_limit".into()).is_truthy() {
            let mut cost: Value = Eqonex::calculate_rate_limiter_cost(self, api.clone(), method.clone(), path.clone(), params.clone(), config.clone(), context.clone());
            self.throttle(cost.clone()).await;
        };
        self.set("last_rest_request_timestamp".into(), self.milliseconds());
        let mut request: Value = Eqonex::sign(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone());
    ...
    async fn request(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value, mut context: Value) -> Value {
        ...
        config = config.or_default(Value::new_object());
        context = context.or_default(Value::new_object());
        return Eqonex::fetch2(self, path.clone(), api.clone(), method.clone(), params.clone(), headers.clone(), body.clone(), config.clone(), context.clone()).await;
    }
    ...
    async fn fetch_ohlcvc(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        Eqonex::load_markets(self, Value::Undefined, Value::Undefined).await;
        let mut trades: Value = Eqonex::fetch_trades(self, symbol.clone(), since.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_trading_view_ohlcv(&self, mut ohlcvs: Value, mut market: Value, mut timeframe: Value, mut since: Value, mut limit: Value) -> Value {
        timeframe = timeframe.or_default(Value::from("1m"));
        let mut result: Value = Eqonex::convert_trading_view_to_ohlcv(self, ohlcvs.clone(), Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined, Value::Undefined);
        return Eqonex::parse_ohlcvs(self, result.clone(), market.clone(), timeframe.clone(), since.clone(), limit.clone());
    }
    ...
    async fn edit_limit_buy_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_sell_order(&mut self, mut id: Value, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::edit_limit_order(self, id.clone(), symbol.clone(), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn edit_limit_order(&mut self, mut id: Value, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::edit_order(self, id.clone(), symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    fn safe_currency(&self, mut currency_id: Value, mut currency: Value) -> Value {
        ...
        if currency_id.clone().is_nonnullish() {
            code = Eqonex::common_currency_code(self, currency_id.to_upper_case());
        };
        return Value::Json(normalize(&Value::Json(json!({
            "id": currency_id,
            "code": code
        ...
        if market_id.clone().is_nonnullish() {
            if self.get("markets_by_id".into()).is_nonnullish() && self.get("markets_by_id".into()).contains_key(market_id.clone()) {
                ...
                if parts_length.clone() == Value::from(2) {
                    ...
                    result.set("base".into(), Eqonex::safe_currency_code(self, result.get(Value::from("baseId")), Value::Undefined));
                    result.set("quote".into(), Eqonex::safe_currency_code(self, result.get(Value::from("quoteId")), Value::Undefined));
    ...
    async fn fetch_partial_balance(&mut self, mut part: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut balance: Value = Eqonex::fetch_balance(self, params.clone()).await;
        return balance.get(part.clone());
    }

    async fn fetch_free_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::fetch_partial_balance(self, Value::from("free"), params.clone()).await;
    }

    async fn fetch_used_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::fetch_partial_balance(self, Value::from("used"), params.clone()).await;
    }

    async fn fetch_total_balance(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::fetch_partial_balance(self, Value::from("total"), params.clone()).await;
    }
    ...
    async fn fetch_funding_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Eqonex::fetch_transaction_fee(self, code.clone(), params.clone()).await;
    }
    ...
    async fn fetch_funding_fees(&mut self, mut codes: Value, mut params: Value) -> Value {
        ...
        return Eqonex::fetch_transaction_fees(self, codes.clone(), params.clone()).await;
    }
    ...
    async fn fetch_transaction_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return Eqonex::fetch_transaction_fees(self, Value::Json(serde_json::Value::Array(vec![code.clone().into()])), params.clone()).await;
    }
    ...
    fn throw_broadly_matched_exception(&mut self, mut broad: Value, mut string: Value, mut message: Value) -> () {
        let mut broad_key: Value = Eqonex::find_broadly_matched_key(self, broad.clone(), string.clone());
    ...
    async fn fetch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        if self.get("has".into()).get(Value::from("fetchTickers")).is_truthy() {
            let mut tickers: Value = Eqonex::fetch_tickers(self, Value::Json(serde_json::Value::Array(vec![symbol.clone().into()])), params.clone()).await;
    ...
    async fn fetch_order_status(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut order: Value = Eqonex::fetch_order(self, id.clone(), symbol.clone(), params.clone()).await;
        return order.get(Value::from("status"));
    }

    async fn fetch_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::fetch_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }

    async fn cancel_unified_order(&mut self, mut order: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::cancel_order(self, self.safe_value(order.clone(), Value::from("id"), Value::Undefined), self.safe_value(order.clone(), Value::from("symbol"), Value::Undefined), params.clone()).await;
    }
    ...
    async fn create_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }
    ...
    async fn create_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("limit"), Value::from("buy"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_limit_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("limit"), Value::from("sell"), amount.clone(), price.clone(), params.clone()).await;
    }

    async fn create_market_buy_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("market"), Value::from("buy"), amount.clone(), Value::Undefined, params.clone()).await;
    }

    async fn create_market_sell_order(&mut self, mut symbol: Value, mut amount: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return Eqonex::create_order(self, symbol.clone(), Value::from("market"), Value::from("sell"), amount.clone(), Value::Undefined, params.clone()).await;
    }
    ...
    async fn create_post_only_order(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        return Eqonex::create_order(self, symbol.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_limit_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut price: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Eqonex::create_order(self, symbol.clone(), Value::from("limit"), side.clone(), amount.clone(), price.clone(), query.clone()).await;
    }
    ...
    async fn create_stop_market_order(&mut self, mut symbol: Value, mut side: Value, mut amount: Value, mut stop_price: Value, mut params: Value) -> Value {
        ...
        return Eqonex::create_order(self, symbol.clone(), Value::from("market"), side.clone(), amount.clone(), Value::Undefined, query.clone()).await;
    }

    fn safe_currency_code(&self, mut currency_id: Value, mut currency: Value) -> Value {
        currency = Eqonex::safe_currency(self, currency_id.clone(), currency.clone());
    ...
    fn parse_tickers(&self, mut tickers: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        if Array::is_array(tickers.clone()).is_truthy() {
            ...
            while i < market_ids.len() {
                ...
                let mut market: Value = Eqonex::safe_market(self, market_id.clone(), Value::Undefined, Value::Undefined);
        ...
        symbols = Eqonex::market_symbols(self, symbols.clone());
        return Eqonex::filter_by_array(self, results.clone(), Value::from("symbol"), symbols.clone(), Value::Undefined);
    }
    ...
    fn parse_deposit_addresses(&self, mut addresses: Value, mut codes: Value, mut indexed: Value, mut params: Value) -> Value {
        ...
        while i < addresses.len() {
            let mut address: Value = extend_2(Eqonex::parse_deposit_address(self, addresses.get(i.into()), Value::Undefined), params.clone());
            result.push(address.clone());
            i += 1;
        };
        if codes.clone().is_nonnullish() {
            result = Eqonex::filter_by_array(self, result.clone(), Value::from("currency"), codes.clone(), false.into());
        };
    ...
    fn parse_funding_rate_histories(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        return Eqonex::filter_by_symbol_since_limit(self, sorted.clone(), symbol.clone(), since.clone(), limit.clone(), Value::Undefined);
    }

    fn safe_symbol(&self, mut market_id: Value, mut market: Value, mut delimiter: Value) -> Value {
        market = Eqonex::safe_market(self, market_id.clone(), market.clone(), delimiter.clone());
    ...
    fn parse_funding_rates(&self, mut response: Value, mut market: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < response.len() {
            let mut parsed: Value = Eqonex::parse_funding_rate(self, response.get(i.into()), market.clone());
    ...
    async fn fetch_trading_fee(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return Eqonex::fetch_trading_fees(self, params.clone()).await;
    }
    ...
    fn parse_open_interests(&self, mut response: Value, mut market: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        while i < response.len() {
            let mut entry: Value = response.get(i.into());
            let mut interest: Value = Eqonex::parse_open_interest(self, entry.clone(), market.clone());
    ...
    async fn fetch_mark_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if self.get("has".into()).get(Value::from("fetchMarkOHLCV")).is_truthy() {
            ...
            return Eqonex::fetch_ohlcv(self, symbol.clone(), timeframe.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
        } else {
    ...
    fn parse_account(&self, mut account: Value) -> Value {
        ...
        if accounts_by_type.contains_key(account.clone()) {
            ...
            let mut market: Value = Eqonex::market(self, account.clone());
            return market.get(Value::from("id"));
        } else {
    ...
    async fn load_markets_helper(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if !reload.is_truthy() && self.get("markets".into()).is_truthy() {
            if !self.get("markets_by_id".into()).is_truthy() {
                return Eqonex::set_markets(self, self.get("markets".into()), Value::Undefined);
            };
            return self.get("markets".into());
        };
        ...
        if self.get("has".into()).get(Value::from("fetchCurrencies")) == true.into() {
            currencies = Eqonex::fetch_currencies(self, Value::Undefined).await;
        };
        let mut markets: Value = Eqonex::fetch_markets(self, params.clone()).await;
        return Eqonex::set_markets(self, markets.clone(), currencies.clone());
    }
    ...
    async fn load_markets(&mut self, mut reload: Value, mut params: Value) -> Value {
        ...
        if reload.is_truthy() && !self.get("reloading_markets".into()).is_truthy() || !self.get("markets_loading".into()).is_truthy() {
            ...
            let mut markets_loading: Value = Eqonex::load_markets_helper(self, reload.clone(), params.clone()).await;
    ...
    async fn dispatch(&mut self, method: Value, params: Value, context: Value) -> Value {
        match method {
            Value::Json(serde_json::Value::String(ref m)) => {
                match m.as_ref() {
                    "publicGetHealth" => Eqonex::request(self, "health".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetInstruments" => Eqonex::request(self, "getInstruments".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetInstrumentPairs" => Eqonex::request(self, "getInstrumentPairs".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetOrderBook" => Eqonex::request(self, "getOrderBook".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetRisk" => Eqonex::request(self, "getRisk".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetTradeHistory" => Eqonex::request(self, "getTradeHistory".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetFundingRateHistory" => Eqonex::request(self, "getFundingRateHistory".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetChart" => Eqonex::request(self, "getChart".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "publicGetGetExchangeInfo" => Eqonex::request(self, "getExchangeInfo".into(), "public".into(), "GET".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostLogon" => Eqonex::request(self, "logon".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostOrder" => Eqonex::request(self, "order".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostCancelOrder" => Eqonex::request(self, "cancelOrder".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostCancelReplaceOrder" => Eqonex::request(self, "cancelReplaceOrder".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetOrder" => Eqonex::request(self, "getOrder".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetOrders" => Eqonex::request(self, "getOrders".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetOrderStatus" => Eqonex::request(self, "getOrderStatus".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetOrderHistory" => Eqonex::request(self, "getOrderHistory".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostUserTrades" => Eqonex::request(self, "userTrades".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetPositions" => Eqonex::request(self, "getPositions".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostCancelAll" => Eqonex::request(self, "cancelAll".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetUserHistory" => Eqonex::request(self, "getUserHistory".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetRisk" => Eqonex::request(self, "getRisk".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetDepositAddresses" => Eqonex::request(self, "getDepositAddresses".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetDepositHistory" => Eqonex::request(self, "getDepositHistory".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetWithdrawRequests" => Eqonex::request(self, "getWithdrawRequests".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostSendWithdrawRequest" => Eqonex::request(self, "sendWithdrawRequest".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    "privatePostGetTransferHistory" => Eqonex::request(self, "getTransferHistory".into(), "private".into(), "POST".into(), params, Value::Undefined, Value::Undefined, Value::Undefined, context).await,
                    _ => unimplemented!(),
                }
            },
            _ => unimplemented!()
...
pub struct EqonexImpl(Value);
impl Exchange for EqonexImpl {}
impl Eqonex for EqonexImpl {}
impl ValueTrait for EqonexImpl {
...
impl EqonexImpl {
    pub fn new(params: Value) -> Self {
        let mut rv = EqonexImpl(match params {
            Value::Json(_) => params,
            _ => Value::new_object()
        });
        ExchangeImpl::init(&mut rv.0);

        let config_entries = Eqonex::describe(&rv);
